...
export function resolveSlotRef(
  ...
  const byId = slots.findIndex((slot) => slotIds.get(slot) === ref)
  if (byId !== -1) return byId

  const named = slots.reduce<number[]>(
    (acc, slot, index) => (slot.name === ref ? [...acc, index] : acc),
    []
  )
  if (named.length > 1) throw new ComfyAmbiguousSlotError(ref, named.length)
  if (named.length === 1) return named[0]

  if (!options.namedSlotsAvailable && isCanonicalIndex(ref)) {
...
export function describeSlotRef(ref: SlotRef): string {
  return typeof ref === 'object' ? `index ${ref.index}` : `'${ref}'`
}
