...
describe('frontend-node resolution', () => {
  ...
  it('reports group membership to resolvers', () => {
    defineReroute()
    ...
    const resolvers = new Map<string, Resolver>([
      [
        ...
        (view) => {
          ...
          candidateGroups = view.nodesOfType('TestSource')[0]?.groups ?? []
          return { out: { omit: true } }
        }
      ]
    ])

    resolveFrontendNodes(graph, resolvers)

    expect(selfGroups.map(({ title }) => title)).toEqual(['Sampling'])
    expect(candidateGroups.map(({ title }) => title)).toEqual(['Sampling'])
  })
  ...
  it('does not expose live nested widget state', () => {
    class Mutable extends LGraphNode {
      constructor() {
        super('Mutable')
        this.addOutput('out', '*')
        this.addWidget(
    ...
    LiteGraph.registerNodeType('Mutable', Mutable)
    cleanups.push(() => LiteGraph.unregisterNodeType('Mutable'))
    const node = spawn('Mutable')
    const resolver: Resolver = ({ self }) => {
      const value = self.widgetValue('config') as {
    ...
    resolveFrontendNodes(graph, new Map([['Mutable', resolver]]))

    expect(node.widgets?.[0].value).toEqual({ nested: { enabled: true } })
  })
...
describe('supply-side resolution', () => {
  ...
  it('rejects a supplier claim on an already-connected input', () => {
    const source = spawn('Broadcaster')
    const sink = spawn('Sink')
    source.connect(0, sink, 0)
    ...
    expect(() => supply(new Map([['Broadcaster', supplier]]))).toThrow(
      /connected input/
    )
  })

  it('rejects supplier claims outside graph topology', () => {
    spawn('Broadcaster')
    const supplier: Supplier = () => [
      { to: { nodeId: 'missing', input: 0 }, from: { output: 0 } }
    ]
  ...
  it('rejects invalid supplier priorities and source slots', () => {
    const sink = spawn('Sink')
    spawn('Broadcaster')
    const invalidPriority: Supplier = () => [
      {
        to: { nodeId: String(sink.id), input: 0 },
        from: { output: 0 },
        priority: Number.NaN
    ...
    expect(() => supply(new Map([['Broadcaster', invalidPriority]]))).toThrow(
      /invalid supplied edge/
    )
    expect(() => supply(new Map([['Broadcaster', invalidOutput]]))).toThrow(
      /nonexistent output/
    )
  })
})
...
describe('a supplier reading its own configuration', () => {
  ...
  it('does not expose live nested property state', () => {
    const properties = { ue_properties: { on: true } }

    supplierSees((view) => {
      const nested = view.self.properties.ue_properties as { on: boolean }
      expect(() => {
        nested.on = false
      }).toThrow()
    }, properties)

    expect(properties.ue_properties.on).toBe(true)
  })
})
