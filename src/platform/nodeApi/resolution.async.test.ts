...
describe('async resolution', () => {
  ...
  it('starts independent resolvers concurrently', async () => {
    const first = spawn('ARelay')
    const second = spawn('ARelay')
    first.addOutput('out', '*')
    second.addOutput('out', '*')
    const releases: (() => void)[] = []
    const resolver: Resolver = () =>
      new Promise((resolve) => {
        releases.push(() => resolve({ out: { omit: true } }))
      })

    const pending = resolveFrontendNodesAsync(
      graph,
      new Map([['ARelay', resolver]])
    )
  ...
  it('aborts pending resolution promptly', async () => {
    const relay = spawn('ARelay')
    relay.addOutput('out', '*')
    const controller = new AbortController()
    let receivedSignal: AbortSignal | undefined
    const resolver: Resolver = ({ signal }) => {
  ...
  it('times out pending resolution', async () => {
    const relay = spawn('ARelay')
    relay.addOutput('out', '*')
    const resolver: Resolver = () => new Promise(() => undefined)
    const pending = resolveFrontendNodesAsync(
      graph,
      new Map([['ARelay', resolver]]),
      { timeoutMs: 25 }
