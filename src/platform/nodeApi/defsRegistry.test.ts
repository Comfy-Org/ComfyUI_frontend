...
describe('defs.extend', () => {
  ...
  describe('selectors', () => {
    ...
    it.for(selectorCases)('%s', ([, selector, shouldMatch]) => {
      const registry = createDefRegistry()
      const apply = vi.fn()
      registry.forMajor(() => comfy.graph.node('1')!).extend(selector, apply)
      registry.applyTo(nodeClass('KSampler'), RAW_DEF)
      expect(apply).toHaveBeenCalledTimes(shouldMatch ? 1 : 0)
    })

    it.for([/^KSam/g, /^KSam/y])(
      'repeats stateful selector %s consistently',
      (selector) => {
        const registry = createDefRegistry()
        const apply = vi.fn()
        registry.forMajor(() => comfy.graph.node('1')!).extend(selector, apply)
  ...
  describe('lifecycle', () => {
    ...
    it('settles property changes independently per node', () => {
      const seen: string[] = []
      const registry = createDefRegistry()
      registry
        .forMajor((id) => comfy.graph.node(id)!)
        .extend('KSampler', (builder) =>
          builder.onPropertyChanged((node, event) => {
            seen.push(node.id)
            if (event.name === 'first') peer.setProperty('second', 2)
          })
        )
      const Generated = nodeClass('KSampler')
      registry.applyTo(Generated, RAW_DEF)
      const first = new Generated()
      const peer = new Generated()
      graph.add(first)
      graph.add(peer)

      first.setProperty('first', 1)

      expect(seen).toEqual([String(first.id), String(peer.id)])
    })
    ...
    it('preserves a legacy property veto', () => {
      const registry = createDefRegistry()
      registry
        .forMajor((id) => comfy.graph.node(id)!)
        .extend('KSampler', (builder) =>
          builder.onPropertyChanged(() => undefined)
        )
      const Generated = nodeClass('KSampler')
      Generated.prototype.onPropertyChanged = () => false
      registry.applyTo(Generated, RAW_DEF)
      const node = new Generated()
      graph.add(node)
      node.setProperty('x', 7)

      expect(node.properties['x']).toBeUndefined()
    })
  ...
  describe('composition', () => {
    ...
    it('stops installed callbacks after unsubscription', () => {
      const created = vi.fn()
      const registry = createDefRegistry()
      const off = registry
        .forMajor((id) => comfy.graph.node(id)!)
        .extend('KSampler', (builder) => builder.onCreated(created))
      const Generated = nodeClass('KSampler')
      registry.applyTo(Generated, RAW_DEF)
      graph.add(new Generated())
    ...
    it('continues installed callbacks after one throws', () => {
      const survivor = vi.fn()
      const registry = createDefRegistry()
      const api = registry.forMajor((id) => comfy.graph.node(id)!)
      api.extend('KSampler', (builder) =>
        builder.onCreated(() => {
          throw new Error('boom')
        })
      )
      api.extend('KSampler', (builder) => builder.onCreated(survivor))
      const Generated = nodeClass('KSampler')
      registry.applyTo(Generated, RAW_DEF)

      expect(() => graph.add(new Generated())).not.toThrow()
      expect(survivor).toHaveBeenCalledOnce()
    })
  })
...
describe('how a node arrived', () => {
  ...
  it('reports a node reconstructed by graph configure as restored', () => {
    const { seen, make } = watched()
    const savedGraph = new LGraph()
    savedGraph.add(make())
    const saved = savedGraph.serialize()
    seen.length = 0
...
describe('a link no single slot fits', () => {
  ...
  it('stops offering a link after unsubscription', () => {
    const registry = createDefRegistry()
    const stop = registry
      .forMajor((id) => comfy.graph.node(id)!)
      .extend('KSampler', (builder) => builder.onUnplacedLink(() => true))
    registry.applyTo(nodeClass('KSampler'), RAW_DEF)

    stop()

    expect(offerUnplacedLink('1', 'KSampler', drop)).toBe(false)
  })
})
...
describe('setSupply', () => {
  ...
  it('reaches a type the backend already registered', () => {
    ...
    void frontendSupplierMap().get('KSampler')!(undefined as never)
  ...
  it('clears a supplier when the definition is reapplied', () => {
    const registry = createDefRegistry()
    const stop = registry
      .forMajor((id) => comfy.graph.node(id)!)
      .extend('KSampler', (builder) => builder.setSupply(() => []))
    registry.applyTo(nodeClass('KSampler'), RAW_DEF)
    stop()

    registry.applyTo(nodeClass('KSampler'), RAW_DEF)

    expect(frontendSupplierMap().has('KSampler')).toBe(false)
  })
})
...
describe('defs.refresh', () => {
  it('asks the host to reload definitions', async () => {
    ...
    setActivePinia(createPinia())
    const graph = new LGraph()
    const refreshDefinitions = vi.fn(async () => {})
    const api = createComfyApi(() => graph, undefined, { refreshDefinitions })
    await api.defs.refresh()

    expect(refreshDefinitions).toHaveBeenCalledTimes(1)
  })

  it('says so when no host is connected', async () => {
    setActivePinia(createPinia())
    const api = createComfyApi(() => new LGraph())

    await expect(api.defs.refresh()).rejects.toThrow(ComfyApiError)
  })
})
...
describe('extend after define', () => {
  it('rejects a registration after definition processing starts', () => {
    setActivePinia(createPinia())
    const api = createComfyApi(() => new LGraph())
  ...
  it('rejects a registration after a backend type was applied', () => {
    setActivePinia(createPinia())
    const graph = new LGraph()
    ...
    expect(() =>
      api.extend('KSampler', (builder) => builder.onCreated(() => {}))
    ).toThrow(/already registered/)
  })

  it('lets a pack extend a type another pack has not defined', () => {
    // The guard was a global latch, so whichever pack called `define()` first
    // made every later pack's `extend()` throw in its module body — and that
    // pack then registered nothing at all. Two example packs were dark in e2e
    // for exactly this reason.
    setActivePinia(createPinia())
    const api = createComfyApi(() => new LGraph())
    api.defs.define({ type: 'PackOneNode' })

    expect(() =>
      api.defs.extend('PackTwoNode', (b) => b.onCreated(() => {}))
    ).not.toThrow()
  })
...
describe('type colours', () => {
  ...
  it('survives a palette load wiping the table', () => {
    ...
    setActivePinia(createPinia())
    const api = createComfyApi(() => new LGraph())
    const undo = api.defs.setTypeColor('PIPE_LINE', '#c9a')
  ...
  it('does not treat a palette-seeded placeholder as the host claiming a type', () => {
    ...
    const undo = api.defs.setTypeColor('XYPLOT', '#abc')
...
describe('preview frames', () => {
  ...
  it('stops delivering frames after unsubscription', () => {
    const graph = new LGraph()
    const comfy = createComfyApi(() => graph)
    const registry = createDefRegistry()
    const seen = vi.fn()
    const stop = registry
      .forMajor((id) => comfy.graph.node(id)!)
      .extend('KSampler', (builder) => builder.onPreview(seen))
    registry.applyTo(nodeClass('KSampler'), RAW_DEF)

    stop()
    deliverPreview('1', 'KSampler', { blob: new Blob(), url: 'blob:x' })
...
describe('connection veto and menu items', () => {
  let graph: LGraph
  let comfy: Comfy
  let peerNode: LGraphNode
  let outputSlot: INodeOutputSlot
  ...
  beforeEach(() => {
    ...
    graph = new LGraph()
    comfy = createComfyApi(() => graph)
    peerNode = new (nodeClass('Peer'))()
    outputSlot = peerNode.addOutput('out', 'IMAGE')
    inputSlot = peerNode.addInput('in', 'IMAGE')
  })
  ...
  it('adds a context menu entry that reaches the node', () => {
    ...
    node.getExtraMenuOptions?.(undefined as never, options)
  ...
  it('computes submenu children per open, from the node as it is now', () => {
    ...
    node.getExtraMenuOptions?.(undefined as never, first)
    ...
    node.getExtraMenuOptions?.(undefined as never, second)
    expect(
      second
...
describe('marking an existing type frontend-only', () => {
  ...
  it('registers a resolver when one is supplied', () => {
    const registry = createDefRegistry()
    const defs = registry.forMajor(() => ({}) as never)
    const resolve = vi.fn(() => ({ '0': { omit: true } }) as never)
    defs.extend('Passthrough', (b) => b.setExecution('frontend', resolve))

    registry.applyTo({ prototype: {} }, { name: 'Passthrough' })

    void frontendResolverMap().get('Passthrough')!(undefined as never)
    expect(resolve).toHaveBeenCalledOnce()
  })

  it('clears a resolver when the definition is reapplied', () => {
    const registry = createDefRegistry()
    const defs = registry.forMajor(() => ({}) as never)
    const stop = defs.extend('Passthrough', (builder) =>
      builder.setExecution('frontend', () => ({ '0': { omit: true } }))
    )
    registry.applyTo({ prototype: {} }, { name: 'Passthrough' })
    stop()
    ...
    expect(frontendResolverMap().has('Passthrough')).toBe(false)
  })
})
