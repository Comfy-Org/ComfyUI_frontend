...
vi.mock(import('@/services/dialogService'), async () => {
  const { fromPartial } = await import('@total-typescript/shoehorn')
  return {
    useDialogService: () =>
      fromPartial<ReturnType<typeof useDialogService>>({ prompt: hostPrompt })
  }
})
...
describe('comfy.ui.addSidebarTab', () => {
  ...
  it('does not remove a replacement through a stale disposer', () => {
    const store = useSidebarTabStore()
    const stale = ui.addSidebarTab(tab())
    store.unregisterSidebarTab('mtb.assets')
    ui.addSidebarTab({ ...tab(), title: 'Replacement' })

    stale()

    expect(
      store.sidebarTabs.find((tab) => tab.id === 'mtb.assets')?.title
    ).toBe('Replacement')
  })
...
describe('comfy.ui.showDialog', () => {
  ...
  it('closes via the returned handle', () => {
    ...
    expect(dialog === undefined || !dialog.visible).toBe(true)
  })
...
describe('comfy.ui.showMenu', () => {
  ...
  // The menu attaches itself to document.body and installs a document keydown
  // handler; Testing Library only removes what it rendered, so an unclosed
  // menu survives into the next test and answers its queries.
