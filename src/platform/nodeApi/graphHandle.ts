...
export function createGraphApi(
  ...
  const api: GraphHandle = {
    ...
    replace(id, type) {
      ...
      const linkScope = graphScopeOf(graph)
      const sources = old.inputs.map((input, index) => {
        const topology = linkStore.getInputSlotLink(linkScope, old.id, index)
        const link =
      ...
      return runAsBatch(() => {
        ...
        for (const source of sources) {
          ...
          const made = source.origin.connect(
            source.originSlot,
            replacement,
            slotOn(replacement, 'input', source.name, source.index)
          )
          if (!made) dropped.push(`input '${source.name}'`)
        }
        for (const target of targets) {
          const made = replacement.connect(
            slotOn(replacement, 'output', target.name, target.index),
            target.target,
            target.targetSlot
          )
