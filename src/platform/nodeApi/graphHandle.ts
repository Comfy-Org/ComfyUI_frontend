...
interface ReplacementTarget {
  readonly name: string
  readonly index: number
  readonly target: LGraphNode
  readonly targetSlot: number
}

function replacementSources(
  graph: LGraph,
  node: LGraphNode
): readonly (ReplacementSource | undefined)[] {
...
function replacementTargets(
  ...
  return node.outputs.flatMap((output, index) =>
    outputLinks(graph, node.id, index).flatMap((link) => {
      const target = graph.getNodeById(link.target_id)
      return target
        ? [{ name: output.name, index, target, targetSlot: link.target_slot }]
        : []
    })
  )
}

function copyNodeState(old: LGraphNode, replacement: LGraphNode): void {
  ...
  for (const key of Object.keys(replacement.properties)) {
    if (Object.hasOwn(old.properties, key)) {
      replacement.properties[key] = old.properties[key]
    }
  }
  const values = new Map(
    old.widgets?.map((widget) => [widget.name, widget.value])
  )
  ...
  replacement.size = [
    Math.max(old.size[0], needed[0]),
    Math.max(old.size[1], needed[1])
  ]
}
...
function replacementSlot(
  ...
  const found =
    side === 'input' ? node.findInputSlot(name) : node.findOutputSlot(name)
  return found === -1 ? index : name
}

function reconnectReplacement(
  replacement: LGraphNode,
  sources: readonly (ReplacementSource | undefined)[],
  ...
  for (const source of sources) {
    ...
    const made = source.origin.connect(
      source.originSlot,
      replacement,
      replacementSlot(replacement, 'input', source.name, source.index)
    )
    if (!made) dropped.push(`input '${source.name}'`)
  }
  for (const target of targets) {
    const made = replacement.connect(
      replacementSlot(replacement, 'output', target.name, target.index),
      target.target,
      target.targetSlot
    )
...
function reportDroppedReplacementLinks(
  id: string,
  type: string,
...
export function createGraphApi(
  ...
  // A local rather than a method, so `replace` does not depend on `this`: the
  // surface is a frozen literal and `const { replace } = comfy.graph` is
  // ordinary use, which would otherwise throw.
  const runAsBatch: GraphHandle['batch'] = (mutations) => {
    ...
    const result = graph ? graph.batchVersionUpdates(run) : run()
    if (
      result !== null &&
      typeof result === 'object' &&
      typeof (result as { then?: unknown }).then === 'function'
    ) {
      throw new ComfyApiError('graph.batch() requires a synchronous callback.')
    }
    return result
  }
  ...
  const api: GraphHandle = {
    ...
    replace(id, type) {
      ...
      const sources = replacementSources(graph, old)
      const targets = replacementTargets(graph, old)

      return runAsBatch(() => {
        graph.add(replacement)
        copyNodeState(old, replacement)
        graph.remove(old)
        reportDroppedReplacementLinks(
          id,
          type,
          reconnectReplacement(replacement, sources, targets)
        )
