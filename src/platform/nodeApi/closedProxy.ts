 ...
 * @knipIgnoreUnusedButUsedByCustomNodes
 */
const COMFY_HANDLE = Symbol.for('comfy.handle')
...
export function createHandleFactory<TTarget>(
  ...
  // Scoped, because a handle's token records the graph it was created in
  // and ids repeat across graphs: without it, asking for node 3 inside a
  // subgraph returns the root graph's handle, and `adopt()` then resolves
  // against the wrong graph.
  const cacheKey = (id: string) =>
    `${namespace}\u0000${identityScope?.() ?? ''}\u0000${id}`
  ...
  function create(id: string): object {
    ...
    const graphId = identityScope?.()
    const token: HandleToken = Object.freeze({
      kind: spec.kind,
      id,
    ...
    const serializeHandle = () => {
      ...
      for (const key of propKeys) {
        if (target !== undefined) out[key] = readable(key, target)
        else if (identity.has(key)) out[key] = identityFallback(key, id)
      }
      out[RESERVED] = target === undefined
      return out
    }

    const callMethod = (key: string, args: never[]): unknown => {
      const target = resolve(id)
      if (target !== undefined) {
    ...
    const methodFor = (key: string): unknown => {
      if (!bound.has(key)) {
        bound.set(key, (...args: never[]) => callMethod(key, args))
      }
    ...
    const propertyValue = (key: string): unknown => {
      const target = resolve(id)
      if (target !== undefined) return props[key].get(target)
      return identity.has(key) ? identityFallback(key, id) : undefined
    }

    const symbolValue = (key: symbol): unknown => {
    ...
    const handler: ProxyHandler<object> = {
      get(_t, key) {
        ...
        if (typeof key === 'symbol') return symbolValue(key)
        if (key === 'toJSON') return serializeHandle
        if (Object.hasOwn(methods, key)) return methodFor(key)
        if (Object.hasOwn(props, key)) return propertyValue(key)
