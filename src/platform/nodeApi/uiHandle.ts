...
type MenuKeyHandler = (root: ContextMenu, event: KeyboardEvent) => void
...
function moveMenuSelection(
  ...
  const next =
    index < 0 ? 0 : Math.max(0, Math.min(index + offset, entries.length - 1))
  focusMenuEntry(menu, next)
}

function openSelectedSubmenu(root: ContextMenu, event: KeyboardEvent): void {
  const menu = deepestMenu(root)
  const index = selectedIndex(menu)
  if (
    index < 0 ||
    menuEntries(menu)[index].getAttribute('aria-haspopup') !== 'true'
  ) {
    return
  }
  event.preventDefault()
  menuEntries(menu)[index].click()
  const submenu = deepestMenu(root)
  if (submenu !== menu) focusMenuEntry(submenu, 0)
}

function closeSelectedSubmenu(root: ContextMenu, event: KeyboardEvent): void {
  const menu = deepestMenu(root)
...
function activateMenuEntry(root: ContextMenu, event: KeyboardEvent): void {
  const menu = deepestMenu(root)
  const index = selectedIndex(menu)
  if (index < 0) return
  event.preventDefault()
...
const menuKeyHandlers: Readonly<Record<string, MenuKeyHandler>> = {
  ArrowDown: (root, event) => moveMenuSelection(root, event, 1),
  ArrowUp: (root, event) => moveMenuSelection(root, event, -1),
  ArrowRight: openSelectedSubmenu,
  ArrowLeft: closeSelectedSubmenu,
  Enter: activateMenuEntry,
  Tab: activateMenuEntry,
  Escape: (root, event) => {
    event.preventDefault()
    root.close()
  }
...
function enableMenuKeyboard(root: ContextMenu): void {
  document.addEventListener(
    'keydown',
    (event) => {
      menuKeyHandlers[event.key]?.(root, event)
    },
...
export function createUiHandle(): UiHandle {
  return {
    ...
    addSidebarTab(def) {
      ...
      const registeredTab = store.sidebarTabs.find((tab) => tab.id === def.id)

      let registered = true
      return () => {
        if (!registered) return
        registered = false
        const currentStore = useSidebarTabStore()
        if (
          currentStore.sidebarTabs.find((tab) => tab.id === def.id) ===
          registeredTab
        ) {
          currentStore.unregisterSidebarTab(def.id)
        }
      }
    },
