...
export function createWidgetHandles(
  ...
  const commit = (
    ...
    programmaticWrites++
    try {
      commitWidgetValue(widget, value, { node })
    } finally {
      programmaticWrites--
    }
  }
      ...
      idMethods: {
        ...
        on: (w, id, ...args) => {
          ...
          if (event === 'textInteraction') {
            ...
            return subscribeWidgetTextInteraction(w, listener, (value) =>
              commit(w, node, value)
            )
          }
  ...
  function ensureCallbackBridge(w: IBaseWidget) {
    ...
    lastNotified.set(w, w.value)
    const original = w.callback
    w.callback = function (this: unknown, value, ...rest) {
      const previous = lastNotified.get(w)
      lastNotified.set(w, value)
      original?.apply(this as never, [value, ...rest] as never)
      notify(w, value, previous)
...
function requireMountableNode(
  ...
  if (node.widgets?.some((widget) => widget.name === def.name)) {
    throw new ComfyApiError(
...
function mountContainer(def: MountDef): HTMLDivElement {
...
function initialMountedData(def: MountDef): MountedData {
  return typeof def.defaultValue === 'object' && def.defaultValue !== null
    ? structuredClone(def.defaultValue)
    : (def.defaultValue ?? '')
}

function connectMountedLifecycle(
  widget: IBaseWidget,
  def: MountDef,
  holdsValue: boolean,
  listeners: ReadonlySet<(value: MountedData) => void>
): void {
  if (def.destroy) {
  ...
  widget.callback = function (this: unknown, value, ...rest) {
    ...
    for (const listener of listeners) listener(value as MountedData)
  }
}

function renderMountedWidget(
  widget: IBaseWidget,
  def: MountDef,
  container: HTMLElement,
  listeners: Set<(value: MountedData) => void>
): void {
...
function mountWidget(node: LGraphNode, def: MountDef): IBaseWidget {
  const container = mountContainer(def)
  const holdsValue = def.defaultValue !== undefined
  let current = initialMountedData(def)
  ...
  const listeners = new Set<(value: MountedData) => void>()
  connectMountedLifecycle(widget, def, holdsValue, listeners)
  renderMountedWidget(widget, def, container, listeners)
  return widget
}
...
export function createWidgetCollection(
  ...
  const collection: WidgetCollection = {
    ...
    mount(def) {
      const node = requireMountableNode(getNode(), def)
      mountWidget(node, def)
      syncWidgetOrder(node)
      return handles.handleFor(nodeId, def.name)
    },
