...
describe('chrome contributions', () => {
  ...
  it('leaves a reclaimed id to whoever holds it now', () => {
    setActivePinia(createTestingPinia({ stubActions: false }))
    const store = useTopbarBadgeStore()
    const stale = addBadgeContribution({ id: 'MyPack.x', text: 'first' })
    stale.remove()
    addBadgeContribution({ id: 'MyPack.x', text: 'second' })

    expect(() => stale.update({ text: 'hijacked' })).toThrow(ComfyApiError)
    stale.remove()
