...
describe('mounted and canvas widgets', () => {
  ...
  it('tells a value-holding mount when the value changed elsewhere', () => {
    ...
    node.widgets![0].callback?.('#abcdef')
...
describe('beforeSerialize', () => {
  ...
  it('rejects a workflow value that JSON cannot serialize', () => {
    widgets.get('seed')!.on('beforeSerialize', (event) => {
      event.setSerializedValue(1n as never)
    })

    expect(saved).toThrow(/non-serializable workflow value/)
  })
