...
function whenDefined<T>(value: T | undefined, use: (value: T) => void): void {
  if (value !== undefined) use(value)
}

function applyNodeDefinition(
  builder: NodeDefBuilder,
  definition: NodeDefinition
): void {
  builder.setExecution(definition.execution ?? 'backend', definition.resolve)
  whenDefined(definition.onCreated, (run) => builder.onCreated(run))
  whenDefined(definition.onExecuted, (run) => builder.onExecuted(run))
  whenDefined(definition.onConfigured, (run) => builder.onConfigured(run))
  whenDefined(definition.onConnectionsChanged, (run) =>
    builder.onConnectionsChanged(run)
  )
  whenDefined(definition.onPropertyChanged, (run) =>
    builder.onPropertyChanged(run)
  )
  whenDefined(definition.onDragOver, (run) => builder.onDragOver(run))
  whenDefined(definition.onDrop, (run) => builder.onDrop(run))
  whenDefined(definition.onRemoved, (run) => builder.onRemoved(run))
  whenDefined(definition.onSerialize, (run) => builder.onSerialize(run))
  whenDefined(definition.onPromptSerialize, (run) =>
    builder.onPromptSerialize(run)
  )
  whenDefined(definition.supply, (supply) => builder.setSupply(supply))
}

interface MenuItemBinding {
  readonly item: NodeMenuItem
  readonly handleFor: Registration['handleFor']
  readonly handleForNode: Registration['handleForNode']
  readonly isActive: () => boolean
}

type InvokeExtension = <T>(hook: string, callback: () => T) => T | undefined
...
function menuItemIsVisible(
  ...
  return (
    !item.when || invoke('menuItem.when', () => item.when!(handle)) === true
  )
}
...
function menuItemChildren(
  ...
  handle: NodeHandle,
  invoke: InvokeExtension
): readonly NodeSubMenuItem[] | undefined {
  const { items } = item
  return typeof items === 'function'
    ? invoke('menuItem.items', () => items(handle))
    : items
}

function submenuOption(
  content: string,
  children: readonly NodeSubMenuItem[],
  ...
  return {
    ...
    submenu: {
      options: children.map((child) => ({
        content: child.label,
        callback: () => invoke('menuItem.run', () => child.run(getHandle()))
      }))
    }
  }
}

function menuOptionFor(
  binding: MenuItemBinding,
  nodeId: string,
  invoke: InvokeExtension
): unknown | undefined {
  const { item, handleFor } = binding
  const handle = handleFor(nodeId)
  if (!menuItemIsVisible(item, handle, invoke)) return undefined
  const content = menuItemContent(item, handle, invoke)
  if (content === undefined) return undefined
  const children = menuItemChildren(item, handle, invoke)
  ...
  if (!children?.length) {
    return {
      content,
      callback: () =>
        invoke('menuItem.run', () => item.run?.(handleFor(nodeId)))
    }
  }
  return submenuOption(content, children, () => handleFor(nodeId), invoke)
}

function appendMenuItems(
  node: LGraphNode,
  options: unknown[],
  items: readonly MenuItemBinding[],
  invoke: InvokeExtension
): void {
  ...
  for (const binding of ordered) {
    ...
    const option = menuOptionFor(binding, String(node.id), invoke)
...
function nodeDefInput(nodeType: string, name: string, spec: unknown) {
  const values = rawComboValues(spec)
  ...
  return Object.freeze({
    name,
    type: rawSlotType(spec),
    ...(localizedName === name ? {} : { localizedName }),
    ...(values ? { values } : {}),
    options: rawSlotOptions(spec)
  })
}
...
function rawNodeOutputs(raw: RawNodeDef) {
  return (raw.output ?? []).map((type, index) =>
    Object.freeze({
...
function nodeDefTitle(raw: RawNodeDef): string {
  return raw.display_name ?? raw.name ?? ''
}

function toNodeDef(raw: RawNodeDef): NodeDef {
  const nodeType = raw.name ?? ''
  const inputs = Object.entries(rawNodeInputs(raw)).map(([name, spec]) =>
    nodeDefInput(nodeType, name, spec)
  )

  return Object.freeze({
    type: nodeType,
    title: nodeDefTitle(raw),
...
function projectionInputNames(value: unknown, nodeType: string): unknown[] {
  const valid =
    typeof value === 'object' &&
    value !== null &&
    !Array.isArray(value) &&
    Object.keys(value).every((key) => key === 'omitInputs') &&
    Array.isArray((value as { omitInputs?: unknown }).omitInputs)
  if (valid) return (value as { omitInputs: unknown[] }).omitInputs
  throw new ComfyApiError(
...
function addProjectedInputNames(
  names: readonly unknown[],
  policy: PromptInputProjectionPolicy,
  nodeType: string,
  omitted: Set<string>
): void {
...
export async function projectedPromptInputOmissions(
  ...
  for (const { run, handleFor, isActive } of policy.projectors) {
    ...
    addProjectedInputNames(
      projectionInputNames(value, nodeType),
      policy,
      nodeType,
      omitted
    )
...
function rawDefinedNode(definition: NodeDefinition): RawNodeDef {
  return {
    name: definition.type,
    display_name: definition.title ?? definition.type,
    category: definition.category ?? 'custom',
    description: definition.description ?? '',
    output: definition.outputs?.map((output) => output.type) ?? [],
    output_name: definition.outputs?.map((output) => output.name) ?? [],
    input: {
      required: Object.fromEntries(
        (definition.inputs ?? []).map((input) => [input.name, [input.type, {}]])
      )
    }
  }
}

function definedNodeClass(definition: NodeDefinition) {
...
export function createDefRegistry(refreshDefinitions?: () => Promise<void>): {
  ...
  const defineType = (
    ...
    const raw = rawDefinedNode(definition)
    const Defined = definedNodeClass(definition)
    ...
    const registration: Registration = {
      ...
      handleForNode,
      active: true,
      apply: (builder) => applyNodeDefinition(builder, definition)
    }
  ...
  const registry = {
    ...
    applyTo(nodeType: { prototype: Partial<LGraphNode> }, raw: unknown) {
      ...
      const menuItems: MenuItemBinding[] = []
      const serialized: {
        ...
        for (const registration of registrations) {
          ...
          const builder: NodeDefBuilder = {
            ...
            onPropertyChanged: (run) =>
              propertyChanges.push({ run, ...binding }),
      ...
      const applied = applyRegistrations()
      if (!applied) {
      ...
      const commitDefinition = () => {
        ...
        if (declaredSuppliers.length === 1) {
          ...
          frontendSuppliers.set(def.type, (view) => {
            ...
            return parts.some((part) => part instanceof Promise)
              ? Promise.all(parts).then((edges) => edges.flat())
              : parts.flatMap((part) => (part instanceof Promise ? [] : part))
          })
        } else {
          frontendSuppliers.delete(def.type)
        }
      ...
      const installLifecycleHooks = () => {
        if (created.length || widgets.length || hidden.length) {
          install('onAdded', (node) => {
            ...
            for (const { run, handleFor, isActive } of created) {
              if (isActive())
                invoke('onCreated', () => run(handleFor(id), event))
            }
          })
        }
        ...
        if (configured.length) {
          install('onConfigure', (node, serialised) => {
            const data = (extensionValue(serialised) ??
      ...
      const installVisualLifecycleHooks = () => {
        if (resized.length) {
          install('onResize', (node, [width, height]) => {
            ...
            for (const { run, handleFor, isActive } of resized) {
              if (isActive())
                invoke('onResized', () => run(handleFor(id), size))
            }
          })
        }
      ...
      const installInteractionHooks = () => {
        ...
        if (beforeConnect.length) {
          ...
          const peerOf = (
            candidate: unknown,
            peerIndex: number | undefined
          ) => {
            const peer = candidate as
        ...
        if (menuItems.length) {
          ...
          ;(nodeType.prototype as Record<string, unknown>).getExtraMenuOptions =
            function (this: LGraphNode, canvas: unknown, options: unknown[]) {
              ...
              appendMenuItems(this, options, menuItems, invoke)
            }
        }
      }
