...
vi.mock(import('@/scripts/api'), async () => {
  ...
  return { api: fromPartial<ComfyApi>({ fetchApi: vi.fn() }) }
})
...
describe('system monitor API', () => {
  ...
  it('normalizes malformed JSON to an invalid snapshot error', async () => {
    vi.mocked(api.fetchApi).mockResolvedValue(
  ...
  it('removes undeclared fields from the public snapshot', async () => {
    vi.mocked(api.fetchApi).mockResolvedValue(
      new Response(
        JSON.stringify({
          ...snapshot,
          private: true,
          cpu: { ...snapshot.cpu, private: true },
          volumes: [{ ...snapshot.volumes[0], private: true }]
        }),
        { status: 200 }
      )
    )
