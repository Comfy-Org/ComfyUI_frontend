...
function applySlotIdentityPatch(
...
function applyPatch(
  ...
  if (!slot) return
  applySlotIdentityPatch(slot, patch)
  applySlotLayoutPatch(slot, patch)
  applySlotColorPatch(slot, patch)
}
...
function setInputWidgetConfig(
  ...
  delete slot.widget[CONFIG]
  slot.widget[GET_CONFIG] = () => inputSpec
  recreateConnectedPrimitive(graph, node, slot)
}

function assertWidgetConfigTarget(
  slot: INodeInputSlot | undefined,
  patch: InputSlotPatch
): void {
  if (
    !patch.widgetConfig ||
    (patch.widget !== null && (slot?.widget || patch.widget !== undefined))
  ) {
    return
  }
  throw new ComfyApiError(
    `Input '${slot?.name ?? ''}' needs a widget before it can receive widgetConfig.`
  )
}

function assertWidgetTarget(
...
function applyInputWidgetPatch(
  graph: LGraph | null | undefined,
  node: LGraphNode | undefined,
  slot: INodeInputSlot | undefined,
  patch: InputSlotPatch
): void {
  if (!slot || patch.widget === undefined) return
  if (patch.widget === null) {
    if (node) clearInputWidget(graph, node, slot)
    return
  }
  if (slot.widget?.name !== patch.widget) {
...
function createInputHandle(
  ...
  const handle: InputSlotHandle = {
    ...
    modify(patch) {
      ...
      assertWidgetConfigTarget(current, patch)
      assertWidgetTarget(node, patch)
      const slot = slotAt()
      applyPatch(slot, patch)
      applyInputWidgetPatch(getGraph(), node, slot, patch)
...
function createOutputHandle(
  ...
  const handle: OutputSlotHandle = {
    ...
    disconnect(targetNodeId) {
      ...
      if (targetNodeId === undefined) return node.disconnectOutput(i)
      const target = graph?.getNodeById(toNodeId(targetNodeId))
      if (!target) {
        throw new ComfyApiError(
          `No node with id '${targetNodeId}' in this graph.`
...
function slotProperties(options?: SlotOptions) {
  ...
  applySlotShape(properties, options)
  applySlotPlacement(properties, options)
  applySlotWidget(properties, options)
  return Object.keys(properties).length ? properties : undefined
}

function applySlotShape(
  properties: Partial<INodeInputSlot>,
  options: SlotOptions
): void {
...
function reorderSlots(
  ...
  const store = useLinkStore()
  const scope = graphScopeOf(graph)
  const patches = reorderedEndpointPatches(
    store,
    scope,
    node,
    side,
  ...
  applyEndpointUpdates(store, scope, patches)

  slots.length = 0
  slots.push(...moved)
}

function reorderedEndpointPatches(
  ...
  for (const [to, name] of names.entries()) {
    ...
    patches.push(...endpointPatches(store, scope, node, side, wasAt, to))
  }
  return patches
}

function applyEndpointUpdates(
  store: ReturnType<typeof useLinkStore>,
  scope: ReturnType<typeof graphScopeOf>,
  patches: readonly EndpointUpdate[]
): void {
...
function endpointPatches(
  ...
  if (side === 'input') {
    const topology = store.getInputSlotLink(scope, node.id, from)
    return topology ? [{ topology, patch: { targetSlot: to } }] : []
  }
  return [...store.getOutputSlotLinks(scope, node.id, from)].map(
    (topology) => ({ topology, patch: { originSlot: to } })
  )
}
...
function createCollection<THandle>(
  ...
  const collection: SlotCollection<THandle> = {
    ...
    reorder(names) {
      ...
      const ambiguous = current.find(
        (name, index) => current.indexOf(name) !== index
      )
      if (ambiguous) {
        throw new ComfyApiError(
          `reorder() cannot address slots by name: '${ambiguous}' is ambiguous.`
        )
      }
      if (
