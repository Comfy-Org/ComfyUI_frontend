...
import { retryWithBackoff, RetriesExhaustedError } from "@/base/common/retry";
...
export const useTeamWorkspaceStore = defineStore("teamWorkspace", () => {
  ...
  /**
   * Core initialization logic - throws on failure for retry handling.
   */
  ...
  async function initialize(): Promise<void> {
    ...
    try {
      await retryWithBackoff(performInitialization, {
        ...
        shouldRetry: (err) => {
          ...
          const isNoWorkspacesError =
            err instanceof Error && err.message === "No workspaces available";
          return !isNoWorkspacesError;
        },
        onRetry: (err, attempt, delay) => {
          const errorMessage = err instanceof Error ? err.message : String(err);
          console.warn(
            `[teamWorkspaceStore] Init failed (attempt ${attempt}), retrying in ${Math.round(delay)}ms: ${errorMessage}`,
          );
        },
      });
      ...
      // Extract the original error if wrapped in RetriesExhaustedError
      const originalError =
        e instanceof RetriesExhaustedError ? e.lastError : e;
      error.value =
