...
describe('useWorkspaceInvoices', () => {
  it('paginates the list into fixed-size pages', () => {
    ...
    const lastPage = Math.ceil(total.value / itemsPerPage)
    page.value = lastPage
    expect(pagedItems.value.length).toBeGreaterThan(0)
    expect(pagedItems.value.length).toBeLessThanOrEqual(itemsPerPage)
  })

  it('filters by event type and snaps the page back into range', async () => {
    const search = ref('')
    const { page, total, itemsPerPage, pagedItems } =
