...
const mocks = vi.hoisted(() => ({
  inMotion: null as unknown as { value: boolean }
}))

vi.mock('./useCoachmarkTarget', async () => {
  const { computed, ref } = await import('vue')
  mocks.inMotion = ref(false)
  return {
    useCoachmarkTarget: () => ({
...
describe('TourSpotlight motion', () => {
  ...
  it('drops position transitions while the target moves and restores them at rest', async () => {
    mocks.inMotion.value = false
