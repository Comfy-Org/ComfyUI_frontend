...
describe('useEduPricing', () => {
  ...
  describe('team EDU pricing (stub)', () => {
    it('is inactive while the backend team-eligibility field is unset', () => {
      mockEduFlag.value = true
      mockIsTeamEduEligible.value = false

      const { isTeamEduPricingActive } = useEduPricing()
      expect(isTeamEduPricingActive.value).toBe(false)
    })

    it('activates once the (currently stubbed) team-eligibility marker is true', () => {
      mockEduFlag.value = true
      mockIsTeamEduEligible.value = true

      const { isTeamEduPricingActive } = useEduPricing()
      expect(isTeamEduPricingActive.value).toBe(true)
    })

    it('is inactive off cloud builds even when eligible', () => {
      mockIsCloud.value = false
      mockEduFlag.value = true
      mockIsTeamEduEligible.value = true
    ...
    it('is inactive when the flag is off even when eligible', () => {
      mockEduFlag.value = false
      mockIsTeamEduEligible.value = true
