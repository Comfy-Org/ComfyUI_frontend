...
function decodeQueryParam(value: string): string | null {
  try {
...
function isSafeInternalRedirectPath(path: string): boolean {
  // Must be a relative in-app path. Disallow protocol-relative URLs ("//evil.com").
  return path.startsWith('/') && !path.startsWith('//')
}

export function getSafePreviousFullPath(query: LocationQuery): string | null {
