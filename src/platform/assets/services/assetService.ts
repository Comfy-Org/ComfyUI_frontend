 ...
 * a namespaced `model_type:<folder>` tag; older backends (and legacy bare tags
 * an otherwise `model_type:`-capable backend may still emit) mint bare tags,
 * which may carry subfolder paths (e.g. `Chatterbox/sub/model`) and group by
 * their top-level segment, matching the asset browser's legacy grouping.
 */
