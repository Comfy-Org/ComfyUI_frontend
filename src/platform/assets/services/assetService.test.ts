...
describe(assetService.getJobAssetId, () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('returns the exact matching asset ID across offset pages', async () => {
    fetchApiMock
      .mockResolvedValueOnce(
        buildResponse({
          assets: [
    ...
    await expect(
      assetService.getJobAssetId('job/1', 'selected.png')
    ).resolves.toBe('asset-3')
  ...
  it('fails closed when identical hashes have a matching asset name', async () => {
    fetchApiMock.mockResolvedValueOnce(
      buildResponse({
        assets: [
          { id: 'asset-1', name: 'first.png', hash: 'shared-hash' },
          { id: 'asset-2', name: 'second.png', hash: 'shared-hash' }
        ],
    ...
    await expect(
      assetService.getJobAssetId('job-1', 'shared-hash', 'second.png')
    ).resolves.toBeNull()
  })

  it('fails closed when identical content hashes cannot be disambiguated', async () => {
    fetchApiMock.mockResolvedValueOnce(
      buildResponse({
        assets: [
          { id: 'asset-1', name: 'first.png', hash: 'shared-hash' },
    ...
    await expect(
      assetService.getJobAssetId('job-1', 'shared-hash')
    ).resolves.toBeNull()
  })

  it('fails closed when the asset listing changes between pages', async () => {
    fetchApiMock
      .mockResolvedValueOnce(
        buildResponse({
          assets: [{ id: 'asset-1', name: 'first.png', hash: 'first-hash' }],
          pagination: { offset: 0, limit: 500, total: 2, has_more: true }
        })
      )
      .mockResolvedValueOnce(
        buildResponse({
          assets: [
            { id: 'asset-2', name: 'second.png', hash: 'selected-hash' }
          ],
          pagination: { offset: 1, limit: 500, total: 1, has_more: false }
        })
      )

    await expect(
      assetService.getJobAssetId('job-1', 'selected-hash')
    ).rejects.toThrow('changed during pagination')
  })

  it('returns null when the job assets endpoint is unavailable', async () => {
    fetchApiMock.mockResolvedValueOnce(
      buildResponse(null, { ok: false, status: 404 })
    )
  ...
  it('throws when the response offset does not match the requested offset', async () => {
    ...
    await expect(
      assetService.getJobAssetId('job-1', 'selected.png')
    ).rejects.toThrow('Invalid job assets pagination offset')
  })
  ...
  it('throws instead of deleting from an unbounded asset listing', async () => {
    fetchApiMock.mockImplementation((input) => {
      ...
      return Promise.resolve(
        buildResponse({
          assets: [{ id: `asset-${offset}`, name: `asset-${offset}.png` }],
          pagination: {
            offset,
            limit: 500,
    ...
    await expect(
      assetService.getJobAssetId('job-1', 'selected.png')
    ).rejects.toThrow('exceeded 1000 batches')
