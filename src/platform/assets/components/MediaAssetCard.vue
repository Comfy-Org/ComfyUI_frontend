...
const sharedOwner = computed(() =>
  asset ? sharing.avatarFor(asset.id) : undefined
)

const { t } = useI18n()

// The role=button aria-label masks descendant text, so the owner badge's
// attribution has to ride along in the card's accessible name.
const cardAriaLabel = computed(() => {
  if (!asset) return t('assetBrowser.ariaLabel.loadingAsset')
  const label = t('assetBrowser.ariaLabel.assetCard', {
    name: getAssetDisplayName(asset),
    type: fileKind.value
  })
  if (!sharedOwner.value) return label
  return `${label}. ${t('mediaAsset.sharedByWorkspace', {
    name: sharedOwner.value.name
  })}`
})
