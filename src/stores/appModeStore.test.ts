...
const mockCanvas = vi.hoisted(() => ({
  state: undefined as { read_only: boolean } | undefined
}))

vi.mock('@/renderer/core/canvas/canvasStore', () => ({
  useCanvasStore: () => ({
    getCanvas: () => mockCanvas.state!
  })
}))
...
describe('appModeStore', () => {
  ...
  beforeEach(() => {
    ...
    mockSettings.reset()
    mockCanvas.state = reactive({ read_only: false })
  ...
  describe('read only canvas sync', () => {
    it('keeps canvas read-only while in select mode', async () => {
      ...
      store.enterBuilder()
      await nextTick()
      mockCanvas.state.read_only = false
      await nextTick()

      expect(mockCanvas.state.read_only).toBe(true)
    })
    ...
    it('stops enforcing read-only after leaving select mode', async () => {
      ...
      expect(mockCanvas.state.read_only).toBe(false)
    })
  })
