...
const mockWorkspaceAuthStore = vi.hoisted(() => ({
  ...
  mintAtLogin: vi.fn(),
  getUnifiedToken: vi.fn(),
  getWorkspaceAuthHeader: vi.fn(),
  getWorkspaceToken: vi.fn(),
  ensureWorkspaceAuthHeader: vi.fn(),
  ensureWorkspaceToken: vi.fn()
}))
...
describe('useAuthStore', () => {
  ...
  beforeEach(() => {
    ...
    mockWorkspaceAuthStore.getUnifiedToken.mockReturnValue(undefined)
    ...
    mockWorkspaceAuthStore.ensureWorkspaceAuthHeader.mockResolvedValue(null)
    mockWorkspaceAuthStore.ensureWorkspaceToken.mockResolvedValue(null)
  ...
  describe('getAuthHeader', () => {
    ...
    it('uses the unified cloud token when enabled', async () => {
      mockFeatureFlags.unifiedCloudAuthEnabled = true
      mockWorkspaceAuthStore.getUnifiedToken.mockReturnValue('unified-token')
    ...
    it('prefers cached workspace auth on cloud', async () => {
  ...
  describe('fetchBalance', () => {
    ...
    it('throws API errors when fetching balance fails', async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        statusText: 'Internal Server Error',
        text: () =>
          Promise.resolve(JSON.stringify({ message: 'Balance unavailable' }))
      })
      ...
      mockFetch.mockImplementation((url: string) => {
        ...
        if (url.endsWith('/customers/credit')) {
          return Promise.resolve({
            ...
            text: () =>
              Promise.resolve(
                JSON.stringify({ message: 'Checkout unavailable' })
              )
          })
        }
        return Promise.reject(new Error('Unexpected API call'))
