...
export class LGraphNode {
   ...
   */
  addProperty(name: string, default_value: unknown, type?: string, extra_info?: Dictionary<unknown>): INodePropertyInfo {
  ...
  computeSize(out?: Size): Size {
    ...
    if (this.widgets?.length) size[0] = Math.max(size[0], LiteGraph.NODE_WIDTH * 1.5)
  ...
  inResizeCorner(canvasX: number, canvasY: number): boolean {
    ...
    return isInsideRectangle(canvasX, canvasY, this.pos[0] + this.size[0] - 15, this.pos[1] + Math.max(this.size[1] - 15, outputs_offset), 20, 20)
  }
  ...
  addWidget(type: string, name: string, value: any, callback: IWidget['callback'], options?: any): IWidget {
    ...
    if (options && typeof options === 'string') options = { property: options }
  ...
  connect(slot: number | string, target_node: LGraphNode, target_slot: ISlotType): LLink | null {
    ...
    if (target_node.onConnectInput?.(targetIndex, output.type, output, this, slot) === false) return null
    if (this.onConnectOutput?.(slot, input.type, input, target_node, targetIndex) === false) return null
    ...
    //create link class
    link_info = new LLink(nextId, input.type || output.type, this.id, slot, target_node.id, targetIndex)
    ...
    this.onConnectionsChange?.(NodeSlotType.OUTPUT, slot, true, link_info, output)

    target_node.onConnectionsChange?.(NodeSlotType.INPUT, targetIndex, true, link_info, input)
    graph.onNodeConnectionChange?.(NodeSlotType.INPUT, target_node, targetIndex, this, slot)
    graph.onNodeConnectionChange?.(NodeSlotType.OUTPUT, this, slot, target_node, targetIndex)
  ...
  disconnectOutput(slot: string | number, target_node?: LGraphNode): boolean {
    ...
    const graph = this.graph
    if (target_node) {
      if (typeof target_node === 'number') target_node = graph.getNodeById(target_node)
      ...
      for (let i = 0, l = output.links.length; i < l; i++) {
        ...
        if (link_info.target_id == target_node.id) {
          ...
          target_node.onConnectionsChange?.(NodeSlotType.INPUT, link_info.target_slot, false, link_info, input)
  ...
  pin(v?: boolean): void {
    this.graph._version++
    this.flags.pinned = v === undefined ? !this.flags.pinned : v
