...
describe('promoted subgraph widgets', () => {
  ...
  it('shows a link indicator only for subgraph input connections', () => {
    ...
    expect(processInteriorWidget(node)?.visible).toBe(true)
    expect(processInteriorWidget(node)?.suppressedByConnection).toBe(true)
  ...
  it.for([
    ...
    ([type, display, hasControl]) => {
      ...
      expect(result?.visible).toBe(display !== undefined)
    }
  )
