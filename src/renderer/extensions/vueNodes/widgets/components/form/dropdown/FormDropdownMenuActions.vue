...
const actionButtonStyle =
  'h-8 bg-node-component-widget-input-surface rounded-lg outline outline-1 outline-offset-[-1px] outline-node-divider transition-all duration-150'

const resetInputStyle = 'bg-transparent border-0 outline-0 ring-0 text-left'

const layoutSwitchItemStyle =
  'size-6 flex justify-center items-center rounded-sm cursor-pointer transition-all duration-150 hover:scale-108 hover:text-text-primary active:scale-95'
    ...
    <label
      :class="
        cn(
          ...
          searchQuery?.trim() !== '' ? 'text-text-primary' : '',
      ...
      <div
        :class="
          cn(
            ...
            'rounded-lg outline outline-offset-[-1px] outline-node-divider'
          )
        "
      >
        <button
      ...
      <button
        :class="
          cn(
            ...
            layoutMode === 'list'
              ? 'bg-node-component-surface-hovered text-text-primary'
              : ''
          )
        "
