...
const canvasMocks = vi.hoisted(() => ({
  store: {
    ...
    linearMode: false,
    currentGraph: null as object | null
  }
}))

vi.mock<unknown>(import('@/renderer/core/canvas/canvasStore'), async () => {
  const { reactive } = await import('vue')
  canvasMocks.store = reactive(canvasMocks.store)
  return { useCanvasStore: () => canvasMocks.store }
})
...
describe('WidgetLegacy', () => {
  ...
  beforeEach(() => {
    canvasMocks.store.canvas.graph.getNodeById.mockReset()
    canvasMocks.store.currentGraph = null
    resolveMock.mockReset()

    mockNode = createMockNode()
    mockWidgetInstance = createMockWidgetInstance()

    canvasMocks.store.canvas.graph.getNodeById.mockReturnValue(mockNode)
  ...
  it('clears the previous widget.width when rebinding to a different widget', async () => {
    ...
    canvasMocks.store.currentGraph = {}
    await nextTick()
