...
// Cast props option to any to bypass strict compiler checks.
// Uses keyword checking as a dual-fallback if layout settings are stripped.
const isRandomizeEnabled = computed(() => {
  const isComponentEnabled = (props.widget?.options as any)?.component === 'SetRandomInt'
  const name = props.widget?.name?.toLowerCase() ?? ''
  const isSeed = name.includes('seed') || name.includes('random')
  return isComponentEnabled || isSeed
})
...
<template>
  <!-- Wrapper with subgrid layout if Randomize button is enabled -->
