...
class LayoutStoreImpl {
  ...
  /** Detaches the operation from the caller and stamps this session's actor. */
  private snapshotOperation(operation: LayoutOperation): LayoutOperation {
    const snapshot = structuredClone(operation)
    snapshot.actor ??= this.currentActor
    return snapshot
  }
