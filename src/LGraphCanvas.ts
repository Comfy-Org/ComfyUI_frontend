...
import type { CanvasDragEvent, CanvasMouseEvent } from "./types/events";
...
export class LGraphCanvas {
                  ...
                  if (e.shiftKey) {
                    if (output.links?.length > 0) {
                      ...
                      for (const linkId of output.links) {
                        ...
                        const linked_node =
                          this.graph._nodes_by_id[link.target_id];
            ...
            for (var i = 0; i < this.visible_links.length; ++i) {
              ...
              if (
                ...
                if (
                  e.shiftKey &&
                  link.path &&
                  this.ctx.isPointInStroke(link.path, e.canvasX, e.canvasY)
                ) {
                  overLink = link;
                } else {
                  continue;
            ...
            if (this.isOverNodeBox(node, e.canvasX, e.canvasY)) {
              ...
              if (
                ...
                if (this.getWidgetLinkType) {
                  ...
                  if (overWidget) {
                    const widgetLinkType = this.getWidgetLinkType(
                      overWidget,
                      node,
                    );
                    if (
                      ...
                      if (
                        firstLink.node.isValidWidgetLink?.(
                          firstLink.output.slot_index,
                          node,
                          overWidget,
                        ) !== false
                      ) {
   ...
   **/
  adjustMouseEvent(
    e: CanvasMouseEvent | CanvasDragEvent | CanvasWheelEvent,
  ): asserts e is CanvasMouseEvent {
    var clientX_rel = 0;
    var clientY_rel = 0;
  ...
  computeVisibleNodes(nodes?: LGraphNode[], out?: LGraphNode[]): LGraphNode[] {
    ...
    for (var i = 0, l = nodes.length; i < l; ++i) {
      ...
      if (
        !overlapBounding(
          this.visible_area,
          n.getBounding(LGraphCanvas.#temp, true),
        )
      ) {
        continue;
      } //out of the visible area
      ...
      if (this.connecting_links) {
        ...
        for (const link of this.connecting_links) {
          ...
          if (connDir == null) {
            ...
            else connDir = link.node.horizontal ? LiteGraph.UP : LiteGraph.LEFT;
          }
  ...
  drawNode(node: LGraphNode, ctx: CanvasRenderingContext2D): void {
    ...
    var out_slot = this.connecting_links
      ? this.connecting_links[0].output
      : null;
  ...
  drawSelectionBounding(
    ...
    switch (shape) {
      ...
      case LiteGraph.CARD_SHAPE: {
        ...
        const cornerRadii =
          isCollapsed || shape === LiteGraph.ROUND_SHAPE
            ? [radius]
            : [radius, 2, radius, 2];
        ctx.roundRect(
          x - padding,
  ...
  drawConnections(ctx: CanvasRenderingContext2D): void {
    ...
    for (var n = 0, l = nodes.length; n < l; ++n) {
      ...
      for (var i = 0; i < node.inputs.length; ++i) {
        ...
        if (
          !overlapBounding(
            LGraphCanvas.#link_bounding,
            LGraphCanvas.#margin_area,
          )
        ) {
          continue;
        }
        ...
        case "combo":
          ...
          if (show_text) {
            ...
            if (w.type == "number") {
              ...
              const labelWidth =
                ctx.measureText(w.label || w.name).width + margin * 2;
              ...
              if (textWidth > availableWidth) {
                ...
                if (availableWidth <= ellipsisWidth) {
                  ...
                  v = `${v}`;
                  const overflowWidth =
                    textWidth + ellipsisWidth - availableWidth;
                  ...
                  if (overflowWidth + charWidthAvg * 3 > availableWidth) {
                    ...
                    const preTruncateCt = Math.floor(
                      (preciseRange - ellipsisWidth) / charWidthAvg,
                    );
                    v = v.substr(0, preTruncateCt);
                  }
                  while (
                    ctx.measureText(v).width + ellipsisWidth >
                    availableWidth
                  ) {
  ...
  // TODO: Not called.  Confirm no consumers, then remove.
  ...
  showSubgraphPropertiesDialogRight(node: LGraphNode) {
    // console.log("showing subgraph properties dialog");
    var that = this;
