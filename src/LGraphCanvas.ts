...
export class LGraphCanvas {
  ...
  static showMenuNodeOptionalInputs(v: unknown, options: INodeInputSlot[], e: MouseEvent, prev_menu: ContextMenu, node: LGraphNode): boolean {
    ...
    options = node.onGetInputs ? node.onGetInputs() : node.optional_inputs
  ...
  static showMenuNodeOptionalOutputs(v: unknown, options: INodeOutputSlot[], e: unknown, prev_menu: ContextMenu, node: LGraphNode): boolean {
    ...
    options = node.onGetOutputs ? node.onGetOutputs() : node.optional_outputs
  ...
  getWidgetAtCursor(node?: LGraphNode): IWidget | null {
    ...
    for (const widget of node.widgets) {
      ...
      if (widget.last_y !== undefined && x >= 6 && x <= widgetWidth - 12 && y >= widget.last_y && y <= widget.last_y + widgetHeight) {
        return widget
      }
    }
  ...
  processMouseDown(e: CanvasPointerEvent): boolean {
    ...
    if (e.which == 1 && !this.pointer_is_double) {
      ...
      if (node && (this.allow_interaction || node.flags.allow_interaction) && !skip_action && !this.read_only) {
        ...
        if (this.allow_interaction && !this.connecting_links && !node.flags.collapsed && !this.live_mode) {
          ...
          if (!skip_action && node.resizable !== false && node.inResizeCorner(e.canvasX, e.canvasY)) {
            this.graph.beforeChange()
        ...
        if (!skip_action) {
          ...
          if (this.selected_group && !this.read_only) {
            ...
            const dist = distance(
              [e.canvasX, e.canvasY],
              [this.selected_group.pos[0] + this.selected_group.size[0], this.selected_group.pos[1] + this.selected_group.size[1]],
            )
      ...
      if (LiteGraph.middle_click_slot_add_default_node) {
        if (node && this.allow_interaction && !skip_action && !this.read_only) {
          //not dragging mouse to connect two slots
          if (!this.connecting_links && !node.flags.collapsed && !this.live_mode) {
            ...
            if (mClikSlot && mClikSlot_index !== false) {
              ...
              const posRef: Point = [!mClikSlot_isOut ? node_bounding[0] : node_bounding[0] + node_bounding[2], e.canvasY - 80]
              // eslint-disable-next-line @typescript-eslint/no-unused-vars
    ...
    if (
      ...
      (ref_window.document.activeElement.nodeName.toLowerCase() != 'input' && ref_window.document.activeElement.nodeName.toLowerCase() != 'textarea')
    ) {
      e.preventDefault()
    }
    e.stopPropagation()
  ...
  processMouseMove(e: CanvasMouseEvent): boolean {
    ...
    if (this.dragging_rectangle) {
      ...
      this.selected_group.resize(e.canvasX - this.selected_group.pos[0], e.canvasY - this.selected_group.pos[1])
      this.dirty_bgcanvas = true
      ...
      if (node) {
        ...
        //Search for corner
        if (this.canvas) {
          this.canvas.style.cursor = node.inResizeCorner(e.canvasX, e.canvasY) ? 'se-resize' : 'crosshair'
        }
      } else {
        //not over a node
        //search for link connector
        ...
        if (this.graph) {
          ...
          if (!node || (w > 10 && h > 10)) {
            ...
            for (const group of groups) {
              ...
              if (
                !isInsideRectangle(pos[0], pos[1], r[0], r[1], r[2], r[3]) ||
                !isInsideRectangle(pos[0] + size[0], pos[1] + size[1], r[0], r[1], r[2], r[3])
              )
                continue
              this.selectedGroups.add(group)
  ...
  isOverNodeBox(node: LGraphNode, canvasx: number, canvasy: number): boolean {
    ...
    return Boolean(isInsideRectangle(canvasx, canvasy, node.pos[0] + 2, node.pos[1] + 2 - title_height, title_height - 4, title_height - 4))
  }
  /**
    ...
    if (node.inputs) {
      for (let i = 0, l = node.inputs.length; i < l; ++i) {
        ...
        if (node.horizontal) {
          ...
          is_inside = isInsideRectangle(canvasx, canvasy, link_pos[0] - 10, link_pos[1] - 10, width, 20)
        }
        if (is_inside) {
          if (slot_pos) {
    ...
    if (node.outputs) {
      for (let i = 0, l = node.outputs.length; i < l; ++i) {
        ...
        if (node.horizontal) {
          ...
          is_inside = isInsideRectangle(canvasx, canvasy, link_pos[0] - 10, link_pos[1] - 10, 40, 20)
        }
        if (is_inside) {
          if (slot_pos) {
            slot_pos[0] = link_pos[0]
    ...
    if (this.graph) {
      ...
      if (this.connecting_links) {
        ...
        for (const link of this.connecting_links) {
          ...
          if (connDir == null) {
            if (link.output) connDir = link.node.horizontal ? LinkDirection.DOWN : LinkDirection.RIGHT
            else connDir = link.node.horizontal ? LinkDirection.UP : LinkDirection.LEFT
          }
          ...
          this.renderLink(ctx, link.pos, highlightPos, null, false, null, link_color, connDir, link.direction ?? LinkDirection.CENTER)

          ctx.beginPath()
          if (connType === LiteGraph.EVENT || connShape === RenderShape.BOX) {
            ctx.rect(link.pos[0] - 6 + 0.5, link.pos[1] - 5 + 0.5, 14, 10)
            ctx.fill()
            ctx.beginPath()
            ...
            ctx.arc(link.pos[0], link.pos[1], 4, 0, Math.PI * 2)
            ctx.fill()
            ctx.beginPath()
  ...
  #renderSnapHighlight(ctx: CanvasRenderingContext2D, highlightPos: Point): void {
    ...
    const gRadius = width < height ? width : width * Math.max(height / width, 0.5)
  ...
  drawButton(
    ...
    hovercolor?: CanvasColour,
    textcolor?: CanvasColour,
  ): boolean {
    const ctx = this.ctx
  ...
  drawNode(node: LGraphNode, ctx: CanvasRenderingContext2D): void {
    ...
    this.drawNodeShape(node, ctx, size, color, bgcolor, node.is_selected)
   ...
   */
  drawNodeShape(node: LGraphNode, ctx: CanvasRenderingContext2D, size: Size, fgcolor: CanvasColour, bgcolor: CanvasColour, selected: boolean): void {
    //bg rect
    ...
    if (render_title || title_mode == TitleMode.TRANSPARENT_TITLE) {
      ...
      } else if (title_mode != TitleMode.TRANSPARENT_TITLE && (node.constructor.title_color || this.render_title_colored)) {
    ...
    for (let n = 0, l = nodes.length; n < l; ++n) {
      ...
      for (let i = 0; i < node.inputs.length; ++i) {
        ...
        if (start_node_slot == -1) {
          ...
          start_node_slotpos = start_node.getConnectionPos(false, start_node_slot, LGraphCanvas.#tempA)
        }
        ...
        if (!overlapBounding(LGraphCanvas.#link_bounding, LGraphCanvas.#margin_area)) continue
        ...
        const end_dir = end_slot.dir || (node.horizontal ? LinkDirection.UP : LinkDirection.LEFT)
  ...
  renderLink(
    ...
    for (let i = 0; i < num_sublines; i += 1) {
      ...
      if (this.links_render_mode == LinkRenderType.SPLINE_LINK) {
        ...
        const l = 15
        path.lineTo(a[0] + start_offset_x * l, a[1] + start_offset_y * l + offsety)
        path.lineTo(b[0] + end_offset_x * l, b[1] + end_offset_y * l + offsety)
    ...
    if (this.ds.scale >= 0.6 && this.highquality_render && end_dir != LinkDirection.CENTER) {
      //render arrow
    ...
    if (flow) {
      ...
      for (let i = 0; i < 5; ++i) {
        ...
        const flowPos = this.computeConnectionPoint(a, b, f, start_dir, end_dir)
        ctx.beginPath()
  ...
  computeConnectionPoint(a: Point, b: Point, t: number, start_dir: number, end_dir: number): number[] {
  ...
  drawExecutionOrder(ctx: CanvasRenderingContext2D): void {
    ...
    for (let i = 0; i < visible_nodes.length; ++i) {
      ...
      ctx.fillStyle = '#FFF'
      ctx.fillText(stringOrEmpty(node.order), node.pos[0] + LiteGraph.NODE_TITLE_HEIGHT * -0.5, node.pos[1] - 6)
    }
    ctx.globalAlpha = 1
  }
  /**
   ...
   **/
  drawNodeWidgets(node: LGraphNode, posY: number, ctx: CanvasRenderingContext2D, active_widget: IWidget) {
            ...
            if (w.type == 'number') {
              ...
              if (w.options.values) {
                ...
                if (typeof values === 'function')
                  // @ts-expect-error
                  values = values()
                if (values && !Array.isArray(values)) v = values[w.value]
              }
  ...
  processNodeWidgets(
    ...
    for (let i = 0; i < node.widgets.length; ++i) {
      ...
      if (!w || w.disabled || w.hidden || (w.advanced && !node.showAdvanced)) continue
      ...
      switch (w.type) {
        ...
        case 'combo': {
          ...
          if (allow_scroll && event.type == LiteGraph.pointerevents_method + 'move' && w.type == 'number') {
            ...
            if (w.type != 'number') values_list = Array.isArray(values) ? values : Object.keys(values)
      ...
      switch (v) {
        ...
        default: {
          ...
          const nodeCreated = that.createDefaultNodeForSlot(
            Object.assign<ICreateNodeOptions, ICreateNodeOptions>(opts, {
    ...
    function refreshHelper() {
      ...
      if (that.onSearchBox) {
        ...
        for (const i in LiteGraph.searchbox_extras) {
          ...
          if ((!options.show_all_if_empty || str) && extra.desc.toLowerCase().indexOf(str) === -1) continue
        ...
        function inner_test_filter(
          type: string,
          optsIn?: number | { inTypeOverride?: string | boolean; outTypeOverride?: string | boolean; skipFilter?: boolean },
        ): boolean {
          optsIn = optsIn || {}
          const optsDef = {
            skipFilter: false,
  ...
  showEditPropertyValue(node: LGraphNode, property: string, options: IDialogOptions): IDialog {
    ...
    function setValue(value: string | number) {
      if (info?.values && typeof info.values === 'object' && info.values[value] != undefined) value = info.values[value]
  ...
  processContextMenu(node: LGraphNode, event: CanvasMouseEvent): void {
    ...
    if (slot) {
      ...
      if (node.getSlotMenuOptions) {
        ...
        if (slot?.output?.links?.length) menu_info.push({ content: 'Disconnect Links', slot: slot })

        const _slot = slot.input || slot.output
        if (_slot.removable) {
          menu_info.push(_slot.locked ? 'Cannot remove' : { content: 'Remove Slot', slot: slot })
        }
        if (!_slot.nameLocked) menu_info.push({ content: 'Rename Slot', slot: slot })
      }
    ...
    function inner_option_clicked(v, options) {
      ...
      if (v.content == 'Remove Slot') {
        ...
        const dialog = that.createDialog("<span class='name'>Name</span><input autofocus type='text'/><button>OK</button>", options)
        const input = dialog.querySelector('input')
        if (input && slot_info) {
