...
app.registerExtension({
  ...
  async setup() {
    ...
    const activeDownloads = new Map<string, { button: HTMLButtonElement; modelInfo: ModelInfo }>()
    ...
    function checkForMissingModelsDialog(element: HTMLElement) {
      ...
      if (!isDialog && element.querySelector) {
        ...
        dialogs.forEach((dialog) => checkForMissingModelsDialog(dialog as HTMLElement))
        return
      }
      ...
      if (entries.length === 0) {
        ...
        textElements.forEach((elem) => {
          ...
          if (text.match(/\w+\.\w+/) && !elem.querySelector('button')) {
            ...
            if (parts.length >= 2 && looksLikeModelName(parts[parts.length - 1])) {
              entries.push({
    ...
    function addDownloadButton(entry: ModelEntry) {
      ...
      btn.onclick = () => startDownload(modelInfo, btn)
    ...
    async function startDownload(modelInfo: ModelInfo, button: HTMLButtonElement) {
      const knownUrl = getKnownUrl(modelInfo.folder, modelInfo.filename)
      ...
      try {
        ...
        if (response.ok) {
          ...
          activeDownloads.set(data.task_id, { button, modelInfo })
          monitorDownload(data.task_id, button)
        } else {
    ...
    async function monitorDownload(taskId: string, button: HTMLButtonElement) {
          ...
          switch (status.status) {
            case 'completed':
              ...
              if ((app as any).refreshComboInNodes) {
                ;(app as any).refreshComboInNodes()
              }
              break
    ...
    function addDownloadAllButton(dialogElement: HTMLElement, modelEntries: ModelEntry[]) {
      // Find dialog footer or create button container
      let buttonContainer: HTMLElement | null = dialogElement.querySelector(
        '.p-dialog-footer, .dialog-footer'
      )
