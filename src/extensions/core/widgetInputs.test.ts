...
describe('PrimitiveNode', () => {
  ...
  describe('refreshComboInNode', () => {
    ...
    it('updates combo options from freshly passed defs', () => {
      const { node, widget } = setupComboNode()

      node.refreshComboInNode(defsWithCombo(FRESH_OPTIONS))

      expect(widget.options.values).toEqual(FRESH_OPTIONS)
    })

    it('preserves existing options when defs and slot config are missing', () => {
      const { node, widget } = setupComboNode()
      node.outputs[0].widget = undefined
    ...
    it('preserves existing options when GET_CONFIG returns undefined', () => {
      const { node, widget } = setupComboNode()
      node.outputs[0].widget = {
        name: TARGET_INPUT_NAME,
    ...
    it('falls back to slot config when defs omit the target node', () => {
      const { node, widget } = setupComboNode()
    ...
    it('resets an invalid value using freshly passed defs', () => {
      const { node, widget } = setupComboNode()
      widget.value = 'removed_value'
    ...
    it('keeps a valid value without firing its callback', () => {
      const { node, widget } = setupComboNode()
    ...
    it('updates combo options from V2 combo specs in defs', () => {
      ...
      node.refreshComboInNode(defsWithV2Combo(FRESH_OPTIONS))

      expect(widget.options.values).toEqual(FRESH_OPTIONS)
    })

    it('propagates an empty option list from defs', () => {
      const { node, widget } = setupComboNode()

      node.refreshComboInNode(defsWithCombo([]))

      expect(widget.options.values).toEqual([])
      expect(widget.value).toBeUndefined()
    })

    it('falls back to input.name when the input has no widget locator', () => {
