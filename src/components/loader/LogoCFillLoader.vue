<template>
  <span
    role="status"
    :class="cn('inline-flex flex-col items-center', colorClass)"
  >
    <svg
      :width="logoWidth"
    ...
    >
      <div class="absolute inset-0 rounded-full bg-current opacity-20" />
      <div
        :class="
          cn(
            'absolute inset-0 rounded-full bg-current',
            !disableAnimation && 'bar-fill'
          )
        "
      />
    </div>
...
const barHeightMap = { sm: 6, md: 10, lg: 12, xl: 16 } as const
const barGapMap = { sm: 16, md: 24, lg: 32, xl: 40 } as const
const colorMap = {
...
const logoWidth = computed(() => Math.round(heightMap[size] * (VB_W / VB_H)))
const barWidth = computed(() => Math.round(logoWidth.value * BAR_RATIO))
const barHeight = computed(() => barHeightMap[size])
const barGap = computed(() => barGapMap[size])
...
<style scoped>
.c-fill-rect {
  animation: c-fill-right 1s cubic-bezier(0.25, 0, 0.3, 1) 0.75s both;
  will-change: transform;
}

@keyframes c-fill-right {
  0% {
    transform: translateX(calc(v-bind(VB_W) * -1px - v-bind(BLEED) * 1px));
  }
  100% {
    transform: translateX(calc(v-bind(BLEED) * -1px));
  }
}

.bar-fill {
  animation: bar-fill-right 2.5s cubic-bezier(0.25, 0, 0.3, 1) forwards;
  transform-origin: left;
}

@keyframes bar-fill-right {
  0% {
    transform: scaleX(0);
  }
  100% {
