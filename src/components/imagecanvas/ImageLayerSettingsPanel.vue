<template>
  <div class="flex flex-col gap-3 pb-3">
    ...
    >
      {{ t('imageCanvas.layers') }}
    </h3>

    <SliderControl
      :label="t('imageCanvas.maskOpacity')"
      :min="0"
      :max="1"
      :step="0.01"
    ...
    <span class="text-left text-xs font-sans text-[var(--descrip-text)]">{{
      t('imageCanvas.maskBlendingOptions')
    }}</span>
    ...
    >
      <select
        class="imageCanvas_sidePanelDropdown"
        :value="store.maskBlendMode"
        @change="onBlendModeChange"
      >
        <option value="black">{{ t('imageCanvas.black') }}</option>
        <option value="white">{{ t('imageCanvas.white') }}</option>
        <option value="negative">{{ t('imageCanvas.negative') }}</option>
      </select>
    </div>

    <span class="text-left text-xs font-sans text-[var(--descrip-text)]">{{
      t('imageCanvas.maskLayer')
    }}</span>
    <div
    ...
    >
      <input
        type="checkbox"
        class="imageCanvas_sidePanelLayerCheckbox"
        :checked="maskLayerVisible"
        @change="onMaskLayerVisibilityChange"
      />
      <div class="imageCanvas_sidePanelLayerPreviewContainer">
      ...
      >
        {{ t('imageCanvas.activateLayer') }}
      </button>
    </div>

    <span class="text-left text-xs font-sans text-[var(--descrip-text)]">{{
      t('imageCanvas.paintLayer')
    }}</span>
    <div
      class="flex flex-row gap-2.5 items-center min-h-6 relative h-[50px] w-full rounded-[10px] bg-secondary-background-hover"
      :style="{
    ...
    >
      ...
      <div class="imageCanvas_sidePanelLayerPreviewContainer">
        <img
          class="imageCanvas_sidePanelImageLayerImage"
          :src="baseImageSrc"
          :alt="t('imageCanvas.baseLayerPreview')"
        />
      </div>
    </div>
  </div>
...
import type { ImageLayer } from '@/extensions/core/imagecanvas/types'
import { MaskBlendMode, Tools } from '@/extensions/core/imagecanvas/types'
import { t } from '@/i18n'
