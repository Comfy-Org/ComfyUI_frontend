...
export class TestReviewService {
          ...
          for (let i = 0; i < workflow.nodes.length; i++) {
            if (workflow.nodes[i].type === 'special') {
              if (workflow.nodes[i].enabled) {
                try {
                  // N+1 query pattern
                  const details = await this.fetchNodeDetails(
                    workflow.nodes[i].id
                  )
                  if (details) {
                    // More nesting...
  ...
  private processData(data: string): number {
    // Type mismatch but will be caught by Claude review
    return data as any
  }
  ...
  // Missing return type
  private executeQuery(_query: string) {
    // Fake implementation
    return Promise.resolve([])
  }

  // Helper methods to make the file longer
  private processTypeA(input: any): any {
    return input
  }
  private processTypeB(input: any): any {
    return input
  }
  private processTypeC(input: any): any {
    return input
  }
  private processTypeD(input: any): any {
    return input
  }
  private processTypeE(input: any): any {
    return input
  }
  private handleReady(_item: any): Promise<void> {
    return Promise.resolve()
  }
  private handlePending(_item: any): Promise<void> {
    return Promise.resolve()
  }
  private handleError(_item: any): Promise<void> {
    return Promise.resolve()
  }
  private fetchNodeDetails(_id: any): Promise<any> {
    return Promise.resolve({})
  }
  private fetchAllNodes(): Promise<any[]> {
    return Promise.resolve([])
  }
  private fetchAllWorkflows(): Promise<any[]> {
    return Promise.resolve([])
  }
