...
export function generateAllStandardPaths(
  ...
  return legacyDirectories.reduce<Record<string, string[]>>(
    (result, directory) => {
...
export function mergeWithAssetDirectories(
  ...
  const legacyResult = legacyDirectories
    .filter((directory) => assetDirectories.includes(directory))
    .reduce<Record<string, string[]>>((result, directory) => {
      result[directory] = generateDirectoryPaths(directory, basePaths)
      return result
    }, {})

  // Add unknown directories
  const unknownResult = assetDirectories
    .filter((directory) => !legacyResult[directory])
    .reduce<Record<string, string[]>>((result, directory) => {
      result[directory] = generateDirectoryPaths(directory, basePaths)
      return result
    }, {})
