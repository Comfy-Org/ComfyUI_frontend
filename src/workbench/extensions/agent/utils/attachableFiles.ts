...
/* json is deliberately picker-only: the panel's drop handler claims a raw File
   drop only when isAgentAttachable approves it, and a bare-dropped workflow
   .json must stay unclaimed so the graph loader (which only runs on an
   unclaimed drop) can still open it. */
const PICKER_ONLY_FORMATS = ['.json', 'application/json'] as const
