...
describe('agentConsentStore in the standalone agent harness', () => {
  beforeEach(() => {
  ...
  it('resolves consent without a Comfy account or team workspace', async () => {
    const store = useAgentConsentStore()

    expect(store).toMatchObject({ isChecking: false, accepted: false })
    await expect(store.ensureScope()).resolves.toBe('standalone')
    await expect(store.load()).resolves.toBe(false)
    expect(useTeamWorkspaceStore().initialize).not.toHaveBeenCalled()
    expect(accountApi.get).not.toHaveBeenCalled()
  })

  it('keeps an accepted decision on this device', async () => {
    await expect(useAgentConsentStore().accept('standalone')).resolves.toBe(
      true
    )

    expect(useAgentConsentStore().accepted).toBe(true)
    expect(localStorage.getItem(stored.key)).toBe('true')
    expect(accountApi.set).not.toHaveBeenCalled()
  })

  it('reads a decision this device already made', async () => {
    localStorage.setItem(stored.key, 'true')

    await expect(useAgentConsentStore().load()).resolves.toBe(true)
  })

  it('refuses a decision made for another identity', async () => {
    const store = useAgentConsentStore()
    await expect(store.accept('account-a/workspace-a')).resolves.toBe(false)
    expect(store.accepted).toBe(false)
  })
})
