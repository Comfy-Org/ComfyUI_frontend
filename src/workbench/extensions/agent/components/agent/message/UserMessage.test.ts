...
describe('UserMessage', () => {
  ...
  // PM-1157 / PM-1158: dragging a non-first batch output (e.g. "layer 2" of a
  // multi-output job) into the composer attaches it with `ref` set to the
  // bare output filename (no content hash — see outputAssetUtil.ts's
  // deliberate omission) and a correct `previewUrl` captured at drop time.
  // splitAttachments discards that previewUrl whenever `ref` is truthy and
  // reconstructs `/view?...&type=input`, which does not resolve to the
  // dragged output. It should prefer `previewUrl` when present instead.
