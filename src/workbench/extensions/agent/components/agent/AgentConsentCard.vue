...
<template>
  <div class="@container w-full max-w-[1040px]">
    <div
      ...
      aria-modal="true"
      :aria-label="title"
      class="bg-agent-surface border-agent-border relative grid max-h-[90dvh] grid-cols-1 overflow-hidden rounded-[14px] border shadow-[0_20px_24px_-4px_rgba(10,13,18,0.4),0_8px_8px_-4px_rgba(10,13,18,0.25)] @2xl:min-h-[543px] @2xl:grid-cols-[555fr_483fr]"
    >
      ...
      <div class="shrink-0 p-2">
        <video
          v-if="videoSrc"
          :src="videoSrc"
          class="aspect-video w-full rounded-[13px] border border-white/20 object-cover @2xl:aspect-auto @2xl:size-full"
          autoplay
          muted
          loop
          playsinline
        ...
        <div
          v-else
          class="text-agent-fg-muted grid aspect-video w-full place-items-center rounded-[13px] border border-white/20 text-xs @2xl:aspect-auto @2xl:size-full"
        >
