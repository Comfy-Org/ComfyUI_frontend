...
describe('AgentPanelRoot attach flow', () => {
  ...
  it('uploads a picked file, stages its ref, and forwards it on the next send', async () => {
    ...
    expect(
      vi.mocked(useTelemetry())!.trackAgentAttachButtonClicked
  ...
  it('warns with the configured server limit when a video exceeds it', async () => {
    ...
    executionErrors.showErrorOverlay.mockClear()
    stubUploadFetch()
    vi.mocked(useTelemetry())!.trackAgentAttachButtonClicked.mockClear()
    renderWithSelectedTarget()
    await nextTick()
...
describe('AgentPanelRoot run approval telemetry', () => {
  it('keeps the timer for a terminal decision after workflow inspection', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock)
    vi.mocked(useTelemetry())!.trackAgentRunApprovalShown.mockClear()
    vi.mocked(useTelemetry())!.trackAgentRunApprovalResolved.mockClear()
    ...
    await nextTick()
    expect(
      vi.mocked(useTelemetry())!.trackAgentRunApprovalShown
    ...
    await vi.waitFor(() =>
      expect(
        vi.mocked(useTelemetry())!.trackAgentRunApprovalResolved
    ...
    await vi.waitFor(() =>
      expect(
        vi.mocked(useTelemetry())!.trackAgentRunApprovalResolved.mock.calls
      ).toEqual([
...
describe('AgentPanelRoot workflow binding', () => {
  ...
  it('reports a selected binding once when the next turn acknowledges it', async () => {
    ...
    await screen.findByRole('button', { name: 'Send' })
    vi.mocked(useTelemetry())!.trackAgentWorkflowBound.mockClear()
    ...
    expect(
      vi.mocked(useTelemetry())!.trackAgentWorkflowBound
    ).toHaveBeenCalledExactlyOnceWith({
      thread_id: 'th-1',
      workflow_id: 'wf-other',
  ...
  it('attributes the thread started after deleting the open chat', async () => {
    ...
    await screen.findByRole('button', { name: 'Send' })
    vi.mocked(useTelemetry())!.trackAgentThreadStarted.mockClear()
    ...
    expect(
      vi.mocked(useTelemetry())!.trackAgentThreadStarted
    ).not.toHaveBeenCalled()
    ...
    await vi.waitFor(() =>
      expect(
        vi.mocked(useTelemetry())!.trackAgentThreadStarted
      ).toHaveBeenCalledExactlyOnceWith({ source: 'history_delete' })
    )
  })
