...
const copyReportLabel = computed(() => {
  if (reportCopyState.value === 'busy') return t(`${i18nKey}.copying`)
  if (reportCopyState.value === 'done') return t(`${i18nKey}.copied`)
  if (reportCopyState.value === 'failed') return t(`${i18nKey}.copyFailed`)
...
async function copyReport() {
  ...
  try {
    ...
    flashReportCopyState(await copyToClipboard(report))
  } catch (error) {
