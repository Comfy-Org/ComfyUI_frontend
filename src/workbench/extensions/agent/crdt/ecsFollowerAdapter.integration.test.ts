...
describe('EcsFollowerAdapter integration', () => {
  ...
  describe('the real PendingDeleteRetentionStore, wired through a real reconcile (P1)', () => {
    // A fresh adapter/follower per call - the retention store is the only
    // thing shared across them, exactly as it survives a follower being
    // disposed and a new one bound in production (ADR CRDT-WRITE-0035).
    function reconcileOnceWithRetention(
      store: PendingDeleteRetentionStore,
      hostUpdate: Uint8Array | null,
      ...
      const adapter = new EcsFollowerAdapter(mutations, {
        pendingDeletes: (workflowId) =>
          store.retainedNodeIds(
            workflowId,
            new Set(Object.keys(follower.doc.getMap('nodes').toJSON())),
            (nodeId) => readNodeItemIdentity(follower.doc, nodeId),
            authoritative
          )
      })
      adapter.bind('wf', follower)
      const update = hostUpdate ?? Y.encodeStateAsUpdate(new Y.Doc())
      follower.applyRemoteUpdate(update)
      adapter.applyFrame({ workflowId: 'wf', seq: 1, update })
      const nodeIds = useNodeDataStore()
        .getGraphNodesFor('root', 'root')
    ...
    it('does not resurrect a retained delete on the first stale frame after a remint that arrived before catch-up', () => {
      ...
      // The identity opSender.ts would have captured at admission time,
      // read from the SAME host doc the later "stale" frame carries -
      // applying its update onto a follower doc preserves this identity.
      const identity = readNodeItemIdentity(hostDoc, '1')
      store.settleBatch([
        { workflowId: 'wf', nodeId: '1', reason: 'confirmed-applied', identity }
      ])

      // The remint-before-catchup window (`follower_replaced` in
      // useAgentCrdtFollower.ts): an empty replacement doc, not
      // authoritative. Nothing to reconcile yet, but the read must not
      // prune the retention a later, real frame will still need.
      expect(reconcileOnceWithRetention(store, null, false)).toEqual([])

      // The first REAL frame lands, authoritative, but still stale - the
      // host has not applied the delete yet, so it still shows node '1'
      // under its original identity. The retained delete must keep
      // suppressing it in the REAL graph's own node list, not just an
      // internal accessor.
      const staleUpdate = Y.encodeStateAsUpdate(hostDoc)
      expect(reconcileOnceWithRetention(store, staleUpdate, true)).toEqual([])
    })

    it('a doc_reset clearing the workflow lets a real reconcile show the recreated node, at the real graph boundary', () => {
      const store = createPendingDeleteRetentionStore()
      const oldHostDoc = mint(
        {
          nodes: [
            { id: 1, type: 'Source', pos: [0, 0], inputs: [], outputs: [] }
          ],
      ...
      onTestFinished(() => oldHostDoc.destroy())
      store.settleBatch([
        {
          workflowId: 'wf',
          nodeId: '1',
          reason: 'confirmed-applied',
          identity: readNodeItemIdentity(oldHostDoc, '1')
        }
      ])

      // Before the reset: the old incarnation is suppressed, as above.
      expect(
        reconcileOnceWithRetention(
          store,
          Y.encodeStateAsUpdate(oldHostDoc),
          true
        )
      ).toEqual([])

      // `doc_reset` (useAgentCrdtFollower.ts's `onDocReset`) clears this
      // workflow's retention outright - the human-authored delete that
      // caused it already carries its own effect, and re-suppressing after
      // a lineage break would just resurrect nothing forever.
      store.clearWorkflow('wf')

      // The new lineage recreates node '1' under a brand new identity. The
      // real graph must now show it, not suppress it - proving both the
      // old incarnation's removal above AND this one's materialization
      // share the same real reconcile path.
      const newHostDoc = mint(
        {
          nodes: [
      ...
      onTestFinished(() => newHostDoc.destroy())
      expect(
        reconcileOnceWithRetention(
          store,
          Y.encodeStateAsUpdate(newHostDoc),
          true
        )
      ).toEqual([toNodeId(1)])
    })
