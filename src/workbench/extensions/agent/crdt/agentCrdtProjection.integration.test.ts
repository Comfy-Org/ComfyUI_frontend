...
function widgetsOf(node: LGraphNode) {
  assert(node.widgets, `test node registers widgets: ${node.title}`)
  return node.widgets
}
 ...
 * to a previously bound workflow tab does. On this branch the projection is
 * the adapter (stores) followed by the materializer sweep (live graph), the
 * same two steps `useAgentCrdtFollower` runs after each applied frame.
 */
