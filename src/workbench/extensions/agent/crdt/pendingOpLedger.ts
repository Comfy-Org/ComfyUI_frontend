...
export type PendingOpState =
  ...
  | 'unprocessed'
  /**
   * Transmitted, then a resend also drew silence: the host may or may not
   * have applied it (ADR-CRDT-RECONCILE-0035). Parked until a same-lineage
   * frame resolves it by a per-kind effect check, a late `doc_ops_result`
   * arrives, or its bounded settlement deadline elapses.
   */
  | 'delivery_unknown'
  /**
   * The bounded settlement deadline elapsed without proof either way. The
   * optimistic projection and identity remain so a late effect or explicit
...
export interface PendingOpLedger<TShadow = unknown> {
  ...
  /**
   * Park ids whose delivery outcome is unknown (a resend also drew silence).
   * Valid from `inflight` only. Returns the ids that did not transition.
   * `reconcileOpsResult` still reconciles a parked id normally if a late
   * result eventually names it (ADR-CRDT-RECONCILE-0035): parking is a
   * fallback for "no result ever comes", not a terminal state.
   */
  markDeliveryUnknown(opIds: readonly string[]): string[]
  /**
   * Mark parked ids as visibly unresolved after their bounded lifetime.
   * Valid from `delivery_unknown` only. The entries remain reconcilable.
   */
  markUnresolved(opIds: readonly string[]): string[]
  /**
   ...
   * whole-batch rollback. Reconciles from `inflight`, `delivery_unknown`, or
   * `unresolved` (a late result for a retained entry).
   */
...
>(): PendingOpLedger<TShadow> {
  ...
  return {
    ...
    markDeliveryUnknown(opIds) {
      const rejected = opIds.filter((opId) => {
        const entry = ledger.get(opId)
        return !entry || entry.state !== 'inflight'
      })
      for (const opId of opIds) {
        const entry = ledger.get(opId)
    ...
    markUnresolved(opIds) {
      const rejected = opIds.filter((opId) => {
        const entry = ledger.get(opId)
        return !entry || entry.state !== 'delivery_unknown'
      })
      for (const opId of opIds) {
        const entry = ledger.get(opId)
        if (entry?.state === 'delivery_unknown') entry.state = 'unresolved'
      }
      return rejected
    },
