...
export type PendingOpState =
   ...
   * have applied it (ADR-CRDT-RECONCILE-0035). Parked until the next
   * same-lineage catch-up resolves it by a per-kind effect check, or until a
   * late `doc_ops_result` arrives and reconciles it like any other inflight
   * entry.
   */
  | 'delivery_unknown'
...
export interface PendingOpLedger<TShadow = unknown> {
   ...
   * whole-batch rollback. Reconciles from `inflight` or `delivery_unknown`
   * (a late result for a parked entry).
   */
