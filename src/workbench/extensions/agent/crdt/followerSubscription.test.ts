...
describe('FE-GAP-1 — a seq jump means a dropped frame and forces a resync', () => {
  ...
  it('surfaces doc_update_error without projecting when the follower rejects an update', () => {
    const { transport, bridge, projected, updateOutcomes } = wire()
    transport.open = true
    bridge.subscribe(WORKFLOW_ID)
  ...
  it('does not consume a sequence until downstream projection accepts it', () => {
    const { transport, bridge } = wire()
    transport.open = true
    bridge.subscribe(WORKFLOW_ID)
    ...
    bridge.addEventListener('doc_update', (event) => {
      ...
      if (projectionAttempts === 1) event.preventDefault()
    })
    const frame = docUpdateFrame(hostDocUpdate(), WORKFLOW_ID, 2)
