...
export class LayoutFollowerBridge extends EventTarget {
  ...
  private readonly onDocUpdate: EventListener = (event) => {
    ...
    // until a newer lineage identity (explicit doc_reset or implied via a
    // higher lineage_seq) replaces the lineage.
    ...
    // follower doc. Only a newer lineage identity may replace it
    // (ADR-GRAPH-DOCUMENT-0024).
    //
