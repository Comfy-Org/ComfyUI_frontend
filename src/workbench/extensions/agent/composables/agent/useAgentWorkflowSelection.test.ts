...
/** `null`: the standalone agent build, which has no cloud workflow index. */
function setup(cloudIndex: (() => Promise<CloudWorkflowEntry[]>) | null) {
  if (cloudIndex === null) vi.stubEnv('VITE_AGENT_STANDALONE', 'true')
  const listCloudWorkflows = cloudIndex ?? vi.fn(async () => [])
