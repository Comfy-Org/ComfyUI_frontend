...
export function useAgentSession(deps: AgentSessionDeps) {
  ...
  const autoAnsweredAskKeys = new Set<string>()
  ...
  function ingestAsk(
    ...
    if (
      ...
      void autoAnswerAsk(event.data.thread_id, event.data.ask_id)
  }

  async function autoAnswerAsk(threadId: string, askId: string): Promise<void> {
    const key = `${threadId}:${askId}`
    if (autoAnsweredAskKeys.has(key)) return
    autoAnsweredAskKeys.add(key)
    if (!(await answerAsk(askId, 'run'))) autoAnsweredAskKeys.delete(key)
  }
