...
describe('useAgentConsent in the standalone agent harness', () => {
  beforeEach(() => {
  ...
  it('opens after local acceptance without signing in or touching the cloud workspace', async () => {
    const onOpen = vi.fn()
  ...
  it('opens without a card once this device has accepted', async () => {
    const first = useAgentConsent().withConsent(vi.fn())
    await startConsent()
    await first
    const onOpen = vi.fn()
