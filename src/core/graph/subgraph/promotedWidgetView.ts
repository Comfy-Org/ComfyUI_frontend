...
export function createPromotedWidgetView(
  ...
  view.draw = function (
    ...
    if (isDOMWidget(resolved.widget)) {
      drawDOMWidgetPlaceholder(ctx, widget_width, y, H)
      return
    }
...
function isDOMWidget(widget: IBaseWidget): boolean {
  return 'element' in widget && !!widget.element
}
...
function drawDOMWidgetPlaceholder(
  ctx: CanvasRenderingContext2D,
  width: number,
  y: number,
  H: number
  ...
  ctx.textAlign = 'center'
  ctx.textBaseline = 'middle'
  ctx.fillText('Enter subgraph to edit', width / 2, y + H / 2)
  ctx.restore()
}
