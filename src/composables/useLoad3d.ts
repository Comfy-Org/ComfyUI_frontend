...
export const useLoad3d = (nodeOrRef: LGraphNode | Ref<LGraphNode | null>) => {
  const nodeRef = 'value' in nodeOrRef ? nodeOrRef : ref(nodeOrRef)
  ...
  const initializeLoad3d = async (containerRef: HTMLElement) => {
    ...
    try {
      ...
      await restoreConfigurationsFromNode(node as LGraphNode)
      ...
      nodeToLoad3dMap.set(node as LGraphNode, load3d)

      const callbacks = pendingCallbacks.get(node as LGraphNode)
      ...
      if (callbacks && load3d) {
        ...
        pendingCallbacks.delete(node as LGraphNode)
      }
  ...
  const restoreConfigurationsFromNode = async (node: LGraphNode) => {
    ...
    if (savedSceneConfig) {
      ...
      load3d.toggleGrid(savedSceneConfig.showGrid)
      load3d.setBackgroundColor(savedSceneConfig.backgroundColor)
      if (savedSceneConfig.backgroundImage) {
        try {
          await load3d.setBackgroundImage(savedSceneConfig.backgroundImage)
        } catch (error) {
          console.error('Failed to restore background image:', error)
          useToastStore().addAlert(
            t('toastMessages.failedToLoadBackgroundImage')
          )
        }
      }
    }
    ...
    if (savedModelConfig) {
      ...
      load3d.setUpDirection(savedModelConfig.upDirection)
      load3d.setMaterialMode(savedModelConfig.materialMode)
    }
    ...
    if (savedLightConfig) {
      ...
      load3d.setLightIntensity(savedLightConfig.intensity)
    }
  ...
  const waitForLoad3d = (callback: Load3dReadyCallback) => {
    const node = toRaw(nodeRef.value)
    if (!node) return
    const existingInstance = nodeToLoad3dMap.get(node as LGraphNode)
    ...
    if (!pendingCallbacks.has(node as LGraphNode)) {
      pendingCallbacks.set(node as LGraphNode, [])
    }
  ...
  const cleanup = () => {
    ...
    const node = toRaw(nodeRef.value)
    if (node && nodeToLoad3dMap.get(node as LGraphNode) === load3d) {
      nodeToLoad3dMap.delete(node as LGraphNode)
    }
