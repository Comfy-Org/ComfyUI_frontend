...
import type {
  ...
  EditorOutputLayer,
  ImageRef
} from '@/stores/imageCanvasDataStore'
...
export function useImageCanvasSaver() {
  const dataStore = useImageCanvasDataStore()
  const editorStore = useImageCanvasStore()
  ...
  const save = async (): Promise<void> => {
    ...
    try {
      ...
      console.error('[ImageCanvasSaver] Save failed:', error)
      throw error
    }
  }
  ...
  async function uploadMask(
    ...
    try {
      ...
      console.warn('[ImageCanvasSaver] Failed to parse upload response:', error)
    }
  ...
  function loadImageFromUrl(url: string): Promise<HTMLImageElement> {
    return new Promise((resolve, reject) => {
      ...
      img.onerror = (error) => {
        console.error('[ImageCanvasSaver] Failed to load image:', url, error)
