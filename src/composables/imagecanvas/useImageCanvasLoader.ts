...
import type { ImageRef, ImageLayer } from '@/stores/imageCanvasDataStore'
...
export function useImageCanvasLoader() {
  ...
  const loadFromNode = async (node: LGraphNode): Promise<void> => {
    ...
    try {
      ...
      console.error('[ImageCanvasLoader]', errorMessage, error)
