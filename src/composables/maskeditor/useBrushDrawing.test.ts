...
describe('useBrushDrawing', () => {
  ...
  describe('handleDrawing', () => {
    ...
    it('should draw delayed hover points if a stroke starts before the frame runs', async () => {
      let callback: FrameRequestCallback | undefined
      vi.stubGlobal(
        'requestAnimationFrame',
        (frameCallback: FrameRequestCallback): number => {
      ...
      await drawing.handleDrawing(
        createPointerEvent({ offsetX: 10, offsetY: 10 })
      )
      await drawing.startDrawing(
        createPointerEvent({ offsetX: 20, offsetY: 20 })
      )
      mockStore.maskCtx!.beginPath = vi.fn()
      callback?.(0)
      await Promise.resolve()
    ...
    it('should continue a source-over stroke while drawing', async () => {
      ...
      await startDrawing(createPointerEvent())
      mockStore.maskCtx!.beginPath = vi.fn()
      await handleDrawing(createPointerEvent({ offsetX: 60, offsetY: 60 }))
      await Promise.resolve()
    ...
    it('should continue an erasing stroke while drawing', async () => {
      mockStore.currentTool = Tools.Eraser
      const { startDrawing, handleDrawing, drawEnd } = useBrushDrawing()
    ...
    it('should erase while drawing with the secondary pointer button', async () => {
      const { startDrawing, handleDrawing, drawEnd } = useBrushDrawing()

      await startDrawing(createPointerEvent())
      await handleDrawing(
        createPointerEvent({ buttons: 2, offsetX: 60, offsetY: 60 })
      )
      await Promise.resolve()
    ...
    it('should log drawing errors when contexts disappear during a stroke', async () => {
      const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
      const { startDrawing, handleDrawing } = useBrushDrawing()
  ...
  describe('canvas visibility restoration', () => {
    it('should finish mask strokes when the mask canvas element is absent', async () => {
    ...
    it('should restore the RGB canvas after drawing on the RGB layer', async () => {
      mockStore.activeLayer = 'rgb'
      mockStore.currentTool = Tools.PaintPen
      mockStore.rgbCanvas!.style.opacity = '0'
      const { startDrawing, drawEnd } = useBrushDrawing()

      await startDrawing(createPointerEvent())
      await drawEnd(createPointerEvent())

      expect(mockStore.rgbCanvas!.style.opacity).toBe('1')
    })
  })
