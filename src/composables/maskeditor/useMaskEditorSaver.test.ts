...
describe('useMaskEditorSaver', () => {
  ...
  it('reports upload failures with the response body', async () => {
    ...
    await expect(save()).rejects.toThrow('Failed to upload clipspace-mask-')
    await expect(save()).rejects.toThrow(': too large')
  })
  ...
  it('reports invalid upload JSON responses', async () => {
    ...
    await expect(save()).rejects.toThrow('bad json')
  })
  ...
  it('defaults missing upload ref fields and skips missing image widget state', async () => {
    mockNode.widgets = [{ name: 'other', value: 'unchanged' }]
    mockNode.widgets_values = ['unchanged']
    mockNode.properties = undefined
    mockNode.graph = undefined
    vi.mocked(api.fetchApi).mockResolvedValue({
      ok: true,
