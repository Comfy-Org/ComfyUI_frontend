...
interface MinimapOption {
  key: MinimapOptionKey
  labelKey: string
}

const MINIMAP_OPTIONS: MinimapOption[] = [
  {
    key: 'Comfy.Minimap.NodeColors',
    labelKey: 'minimap.nodeColors'
  },
  {
    key: 'Comfy.Minimap.ShowLinks',
    labelKey: 'minimap.showLinks'
  },
  {
    key: 'Comfy.Minimap.ShowGroups',
    labelKey: 'minimap.showGroups'
  },
  {
    key: 'Comfy.Minimap.RenderBypassState',
    labelKey: 'minimap.renderBypassState'
  },
  {
    key: 'Comfy.Minimap.RenderErrorState',
    labelKey: 'minimap.renderErrorState'
  }
]
...
export function useMinimap() {
  ...
  const optionRefs = ref<Record<string, boolean>>({})

  const initializeOptions = () => {
    MINIMAP_OPTIONS.forEach((option) => {
      optionRefs.value[option.key] = settingStore.get(option.key)
    })
  }

  const updateOption = async (key: MinimapOptionKey, value: boolean) => {
    optionRefs.value[key] = value
  ...
  const optionsList = computed(() => {
    return MINIMAP_OPTIONS.map((option) => ({
      ...option,
      value: optionRefs.value[option.key]
    }))
  })
