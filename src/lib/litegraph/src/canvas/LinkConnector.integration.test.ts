  ...
  validateLinkIntegrity: async ({ graph }, use) => {
    await use(() => {
      ...
      for (const link of graph._links.values()) {
        expect(
          graph.getNodeByRawId(link!.origin_id)?.outputs[link!.origin_slot]
            .links
        ).toContain(link.id)
        expect(
          graph.getNodeByRawId(link!.target_id)?.inputs[link!.target_slot].link
        ).toBe(link.id)
      }
      ...
      for (const link of graph.floatingLinks.values()) {
        if (isFloatingNodeId(link.target_id)) {
          ...
          expect(link.origin_slot).not.toBe(-1)
          expect(link.target_slot).toBe(-1)
          const outputFloatingLinks = graph.getNodeByRawId(link.origin_id)
          ...
          expect(link.origin_slot).toBe(-1)
          expect(link.target_slot).not.toBe(-1)
          const inputFloatingLinks = graph.getNodeByRawId(link.target_id)
...
describe('LinkConnector Integration', () => {
  ...
  describe('Moving input links', () => {
    test('Should move input links', ({ graph, connector }) => {
      ...
      const hasInputNode = graph.getNodeByRawId(2)!
      const disconnectedNode = graph.getNodeByRawId(9)!
    ...
    test('Should drop floating links when both sides are disconnected', ({
      ...
      const floatingOutNode = graph.getNodeByRawId(1)!
      ...
      graph.getNodeByRawId(2)!.disconnectInput(0, true)
      expect(graph.floatingLinks.size).toBe(1)

      graph.getNodeByRawId(3)!.disconnectInput(0, false)
      expect(graph.floatingLinks.size).toBe(0)
    ...
    test('Should prevent node loopback when dropping on node', ({
      ...
      const hasOutputNode = graph.getNodeByRawId(1)!
      const hasInputNode = graph.getNodeByRawId(2)!
      const hasInputNode2 = graph.getNodeByRawId(3)!
  ...
  describe('Moving output links', () => {
    ...
    test('Should drop floating links when both sides are disconnected', ({
      ...
      graph.getNodeByRawId(3)!.disconnectInput(0, true)
      expect([...graph.reroutes]).toEqual(reroutesBeforeTest)
    ...
    test('Should support moving multiple output links to a floating reroute', ({
      ...
      const manyOutputsNode = graph.getNodeByRawId(4)!
    ...
    test('Should prevent node loopback when dropping on node', ({
      ...
      expect(hasInputNode.getOutputNodes(0)).toEqual([graph.getNodeByRawId(3)])
  ...
  describe('Floating links', () => {
    ...
    test('Dropping a floating input link onto input slot disconnects the existing link', ({
      ...
      const floatingInputNode = graph.getNodeByRawId(6)!
    ...
    test('Allow reroutes to be used as manual switches', ({
      ...
      const targetNode = graph.getNodeByRawId(2)!
  ...
  test.for<ReconnectTestData>([
    ...
    (
      ...
      if (testFloatingInputs) {
        ...
        graph.getNodeByRawId(4)!.disconnectOutput(0)
      }
