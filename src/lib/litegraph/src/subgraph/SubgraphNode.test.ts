...
describe('SubgraphNode Synchronization', () => {
  ...
  it('keeps a pre-flag serialized instance label over the interior label', () => {
    const subgraph = createTestSubgraph({
      inputs: [{ name: 'seed', type: 'INT' }]
    })
    ...
    // Workflows saved before _labelCustomized existed carry the instance
    // label with no flag field at all.
    const serializedInput = instance.inputs?.[0] as
      | Record<string, unknown>
      | undefined
    delete serializedInput?._labelCustomized
    if (serializedInput) serializedInput.label = 'My Seed'
    subgraphNode.configure(instance)
    subgraphNode.arrange()
