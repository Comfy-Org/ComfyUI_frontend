...
describe('SubgraphOutput', () => {
  ...
  it('connects node outputs to subgraph outputs', () => {
    ...
    const link = subgraph.outputs[0].connect(output, sourceNode, 'reroute-1')
    ...
    expect(link?.parentId).toBe('reroute-1')
  ...
  it('validates output slots and subgraph inputs as targets', () => {
    ...
    const output = subgraph.outputs[0]
    const imageOutput = { name: 'image', type: 'IMAGE', links: [] }
    const latentOutput = { name: 'latent', type: 'LATENT', links: [] }
    const imageInput = { name: 'image', type: 'IMAGE', link: null }

    expect(output.isValidTarget(imageOutput as INodeOutputSlot)).toBe(true)
    expect(output.isValidTarget(latentOutput as INodeOutputSlot)).toBe(false)
