...
describe('LGraphCanvas selectOnly', () => {
  ...
  it.fails('does not emit selection changes for refused non-node picks', () => {
    const { canvas } = createHarness()
    const group = new LGraphGroup('Group')
    ...
    Reflect.apply(canvas.processSelect, canvas, [group, undefined, false, true])
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('aborts an active item drag when selection-only mode is armed', () => {
    const { canvas, firstNode } = createHarness()
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('aborts an active link gesture when selection-only mode is armed', () => {
    const { canvas } = createHarness()
    canvas.pointer.isDown = true
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('does not resize groups', () => {
  ...
  it('retains the replacing live-select when disabled', () => {
    ...
    Reflect.apply(canvas['handleLiveSelect'], canvas, [
      {
        canvasX: 580,
        canvasY: 200,
        shiftKey: false,
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('a gesture that started while picking stays additive after the mode ends', () => {
    const { canvas, firstNode, secondNode } = createHarness()
    LiteGraph.leftMouseClickBehavior = 'select'
    canvas.select(firstNode)
    canvas.selectOnly = true
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('marquee-selects additively under the select setting while picking', () => {
    const { canvas, firstNode, secondNode } = createHarness()
    LiteGraph.leftMouseClickBehavior = 'select'
    canvas.select(firstNode)
    canvas.selectOnly = true
    ...
    Reflect.apply(canvas['_handleMultiSelect'], canvas, [
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('live-selects additively while picking', () => {
    const { canvas, firstNode, secondNode } = createHarness()
    LiteGraph.leftMouseClickBehavior = 'select'
    canvas.select(firstNode)
    canvas.selectOnly = true
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('does not clone nodes on alt-click', () => {
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('does not insert reroutes on alt-click over a link', () => {
    const { canvas, graph } = createHarness()
    const linkSegment = {
      id: 1,
      path: {} as Path2D,
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('does not open the link menu from the link marker', () => {
    const { canvas } = createHarness()
    const linkSegment = {
      id: 1,
      path: {} as Path2D,
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('does not reorder nodes on a node click while picking', () => {
    const { canvas, firstNode } = createHarness()
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('defence in depth: does not emit graph change events when picking starts on an item', () => {
    const { canvas, firstNode } = createHarness()
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('a pan-armed click cannot wipe the picks after the mode ends', () => {
    const { canvas, firstNode } = createHarness()
    canvas.select(firstNode)
    canvas.selectOnly = true
    const event = { canvasX: 700, canvasY: 500 } as CanvasPointerEvent
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('a marquee-armed click cannot wipe the picks after the mode ends', () => {
    const { canvas, firstNode } = createHarness()
    LiteGraph.leftMouseClickBehavior = 'select'
    canvas.select(firstNode)
    canvas.selectOnly = true
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('a node-armed click keeps picking additive after the mode ends', () => {
    const { canvas, firstNode, secondNode } = createHarness()
    canvas.select(firstNode)
    canvas.selectOnly = true
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('a picking click does not select a group after the mode ends', () => {
    const { canvas, graph } = createHarness()
    const group = new LGraphGroup('Group')
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('a picking classic marquee does not select a group after the mode ends', () => {
    const { canvas, graph } = createHarness()
    const group = new LGraphGroup('Group')
    group._bounding.set([600, 300, 100, 100])
    graph.add(group)
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('a picking live marquee does not select a group after the mode ends', () => {
    const { canvas, graph } = createHarness()
    const group = new LGraphGroup('Group')
    group._bounding.set([600, 300, 100, 100])
    graph.add(group)
  ...
  // INV-CANVAS-01/03: remove .fails when this select-only gesture invariant passes.
  it.fails('live-selects additively through the full gesture without churning', () => {
