...
import { asNodeId, LGraph, LGraphNode } from '@/lib/litegraph/src/litegraph'
...
describe('BaseWidget store integration', () => {
  ...
  describe('metadata properties after registration', () => {
    ...
    it('writes to store when registered', () => {
      ...
      const state = store.getWidget(
        widgetId(graph.id, asNodeId(1), 'writeWidget')
      )
    ...
    it('syncs value with store', () => {
      ...
      widget.value = 99
      expect(
        store.getWidget(widgetId(graph.id, asNodeId(1), 'valueWidget'))?.value
      ).toBe(99)

      const state = store.getWidget(
        widgetId(graph.id, asNodeId(1), 'valueWidget')
      )!
      state.value = 55
  ...
  describe('automatic registration via setNodeId', () => {
    it('registers widget with all metadata', () => {
      ...
      const state = store.getWidget(
        widgetId(graph.id, asNodeId(1), 'autoRegWidget')
      )
      expect(state).toBeDefined()
      expect(state?.nodeId).toBe(asNodeId(1))
    ...
    it('registers widget with default metadata values', () => {
      ...
      const state = store.getWidget(
        widgetId(graph.id, asNodeId(1), 'defaultsWidget')
      )
      expect(state).toBeDefined()
    ...
    it('registers widget value accessible via getWidget', () => {
      ...
      expect(
        store.getWidget(widgetId(graph.id, asNodeId(1), 'valuesWidget'))?.value
      ).toBe(77)
    })
  })
  ...
  describe('fallback behavior', () => {
    ...
    it('handles undefined values correctly', () => {
      ...
      const state = store.getWidget(
        widgetId(graph.id, asNodeId(1), 'testWidget')
      )
      expect(state?.disabled).toBe(false)
    })
  })
