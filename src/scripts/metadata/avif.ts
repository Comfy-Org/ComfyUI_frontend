...
function findBox(dataView: DataView, start: number, end: number, type: string): { start: number, end: number } | null {
    let offset = start;
    while (offset < end) {
    ...
    return new Promise<Record<string, string>>((resolve) => {
        ...
        reader.onload = (event) => {
            ...
            for (let i = 0; i < entryCount; i++) {
                ...
                const infeType = String.fromCharCode(...new Uint8Array(buffer, currentOffset + 4, 4));
                ...
                if (infeType === 'infe') {
                    ...
                    if (infeVersion >= 2) {
                        ...
                        if (infeVersion === 2) {
                            ...
                            itemTypeStr = String.fromCharCode(...new Uint8Array(buffer, infeContentStart + 4, 4));
                        } else { // version 3
                            itemId = dataView.getUint32(infeContentStart);
                            itemTypeStr = String.fromCharCode(...new Uint8Array(buffer, infeContentStart + 6, 4));
                        }
            ...
            for (let i = 0; i < ilocItemCount; i++) {
                ...
                if (currentItemId === exifItemId) {
                    for (let j = 0; j < extentCount; j++) {
                        ...
                        switch (offsetSize) {
                            case 1: currentExtentOffset = dataView.getUint8(ilocParseOffset); break;
                            case 2: currentExtentOffset = dataView.getUint16(ilocParseOffset); break;
                            case 4: currentExtentOffset = dataView.getUint32(ilocParseOffset); break;
                            case 8: currentExtentOffset = Number(dataView.getBigUint64(ilocParseOffset)); break;
                        }
                        ilocParseOffset += offsetSize;

                        let currentExtentLength = 0;
                        switch (lengthSize) {
                            case 1: currentExtentLength = dataView.getUint8(ilocParseOffset); break;
                            case 2: currentExtentLength = dataView.getUint16(ilocParseOffset); break;
                            case 4: currentExtentLength = dataView.getUint32(ilocParseOffset); break;
                            case 8: currentExtentLength = Number(dataView.getBigUint64(ilocParseOffset)); break;
                        }
                ...
                ilocParseOffset = extentsStartOffset + extentCount * (indexSize + offsetSize + lengthSize);
            }
            ...
            if (exifOffset !== -1 && exifLength !== -1) {
                ...
                for (let i = 0; i < itemData.length - 4; i++) {
                    if (
                        (itemData[i] === 0x4d && itemData[i + 1] === 0x4d && itemData[i + 2] === 0x00 && itemData[i + 3] === 0x2a) || // MM*
                        (itemData[i] === 0x49 && itemData[i + 1] === 0x49 && itemData[i + 2] === 0x2a && itemData[i + 3] === 0x00)    // II*
                    ) {
                        tiffHeaderOffset = i;
                        break;
                    }
