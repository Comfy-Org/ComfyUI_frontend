...
// authStore (this branch's version, pre-firebaseIdentity-port) reaches
// Firebase directly through vuefire + firebase/auth at store setup, so those
// are mocked just enough that constructing it never reaches real Firebase.
...
describe('api.fetchApi', () => {
  ...
  describe('post-401 retry timeout budget', () => {
    beforeEach(() => {
      ...
      // and firebase/auth are stubbed above so constructing them never reaches
      // real Firebase, and their actions stay real functions we override below.
