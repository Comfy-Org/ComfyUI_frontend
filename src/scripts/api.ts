...
export class ComfyApi extends EventTarget {
  ...
  async getUserData(file: string, options?: RequestInit) {
    return this.fetchApi(
      `/userdata/file?file=${encodeURIComponent(file)}`,
      options
    )
  }
  ...
  async storeUserData(
    ...
    const resp = await this.fetchApi(
      `/userdata/file?file=${encodeURIComponent(file)}&overwrite=${options.overwrite}&full_info=${options.full_info}`,
      {
        method: 'POST',
  ...
  async moveUserData(
    ...
    const resp = await this.fetchApi(
      `/userdata/file/move?source=${encodeURIComponent(source)}&dest=${encodeURIComponent(dest)}&overwrite=${options?.overwrite}`,
      {
        method: 'POST'
      }
    )
