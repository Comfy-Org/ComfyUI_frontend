...
describe('ComfyApp', () => {
  ...
  describe('loadGraphData', () => {
    ...
    it('reports a pre-configure extension failure and resolves false', async () => {
      app.canvasElRef.value = document.createElement('canvas')
      Reflect.set(app, 'rootGraphInternal', new LGraph())
      const showDialog = vi.spyOn(useDialogStore(), 'showDialog')
      mockExtensionService.invokeExtensionsAsync.mockImplementation(
        async (hook: string) => {
          if (hook === 'beforeConfigureGraph') throw new Error('bad extension')
        }
      )
    ...
    it('reports validation rejection and permits a subsequent load', async () => {
      app.canvasElRef.value = document.createElement('canvas')
      Reflect.set(app, 'rootGraphInternal', new LGraph())
      mockSettingStore.get.mockImplementation((key: string) =>
        [
      ...
      const showDialog = vi.spyOn(useDialogStore(), 'showDialog')
      mockValidateWorkflow.mockRejectedValueOnce(new Error('validation failed'))
