...
describe('templateRankingStore', () => {
  ...
  describe('computeDefaultScore', () => {
    it('uses default searchRank of 5 when not provided', () => {
      const store = useTemplateRankingStore()
      const score = store.computeDefaultScore(
        'template',
        '2024-01-01',
        undefined
      )
    ...
    it('high searchRank (10) boosts score', () => {
      const store = useTemplateRankingStore()
      const lowRank = store.computeDefaultScore('template', '2024-01-01', 1)
      const highRank = store.computeDefaultScore('template', '2024-01-01', 10)
      expect(highRank).toBeGreaterThan(lowRank)
    })

    it('low searchRank (1) demotes score', () => {
      const store = useTemplateRankingStore()
      const neutral = store.computeDefaultScore('template', '2024-01-01', 5)
      const demoted = store.computeDefaultScore('template', '2024-01-01', 1)
      expect(demoted).toBeLessThan(neutral)
    })

    it('searchRank difference is significant', () => {
      const store = useTemplateRankingStore()
      const rank1 = store.computeDefaultScore('template', '2024-01-01', 1)
      const rank10 = store.computeDefaultScore('template', '2024-01-01', 10)
  ...
  describe('computePopularScore', () => {
    it('does not use searchRank', () => {
      ...
      const score1 = store.computePopularScore('template', '2024-01-01')
      const score2 = store.computePopularScore('template', '2024-01-01')
    ...
    it('newer templates score higher', () => {
      const store = useTemplateRankingStore()
      const today = new Date().toISOString().split('T')[0]
      const oldScore = store.computePopularScore('template', '2020-01-01')
      const newScore = store.computePopularScore('template', today)
      expect(newScore).toBeGreaterThan(oldScore)
    })
  })

  describe('getUsageScore', () => {
    it('returns 0 for unknown template', () => {
      const store = useTemplateRankingStore()
      expect(store.getUsageScore('unknown_template')).toBe(0)
    })
  })
  ...
  describe('searchRank edge cases', () => {
    it('handles searchRank of 0 (should still work, treated as very low)', () => {
      const store = useTemplateRankingStore()
      const score = store.computeDefaultScore('template', '2024-01-01', 0)
    ...
    it('handles searchRank above 10 (clamping not enforced, but works)', () => {
      const store = useTemplateRankingStore()
      const rank10 = store.computeDefaultScore('template', '2024-01-01', 10)
      const rank15 = store.computeDefaultScore('template', '2024-01-01', 15)
      expect(rank15).toBeGreaterThan(rank10)
    })

    it('handles negative searchRank', () => {
      const store = useTemplateRankingStore()
      const score = store.computeDefaultScore('template', '2024-01-01', -5)
