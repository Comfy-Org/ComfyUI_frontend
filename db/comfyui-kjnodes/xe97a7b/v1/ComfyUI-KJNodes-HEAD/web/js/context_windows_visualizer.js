...
// The onConfigure restore used to be chained onto the instance from inside
// onNodeCreated so it could close over `state`; registered once per type now,
// it reaches that closure through here. Dropped in onRemoved.
const restores = new Map();

comfy.defs.extend('ContextWindowsVisualizerKJ', (b) => {
    b.onCreated((node) => {
        ensureStyles();
        node.setSizeConstraints({ minWidth: 560, minHeight: 480 });
        ...
        node.widgets.mount({
            name: 'context_windows_viz',
            render(container) { container.appendChild(root); },
            // A mounted element owns its listeners, timers and observers.
            destroy() {
                stopPlayback();
                ro.disconnect();
        ...
        const getWidgetValue = (name, fallback) => {
            ...
            return w ? w.getValue() : fallback;
        };

        const persist = () => {
            node.setProperty('cw_viz', { step: state.step, steps: state.steps });
        };
        ...
        restores.set(node.id, function () {
            const saved = node.getProperty('cw_viz');
            if (saved) {
    ...
    b.onConfigured((node) => {
        restores.get(node.id)?.();
    });

    // Teardown moved into the mounted widget's destroy(); only the per-node
    // entry is left to release here.
    b.onRemoved((node) => {
        restores.delete(node.id);
    });
});
