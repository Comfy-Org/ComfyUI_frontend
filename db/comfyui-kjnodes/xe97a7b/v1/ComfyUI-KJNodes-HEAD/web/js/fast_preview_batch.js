...
// Per-node view, keyed by node id. onExecuted/onConfigured used to be chained
// onto the instance from inside onNodeCreated so they could close over the DOM
// this file builds; they are registered once per type now and reach that
// closure through here instead. Dropped in onRemoved.
const views = new Map();
...
comfy.defs.extend('FastPreviewBatch', (b) => {
    b.onCreated((node) => {
        ensureStyles();
        node.setSize({ width: 520, height: 520 });
        ...
        // A mounted element owns its listeners, timers and observers, so the
        // teardown that used to hang off onRemoved belongs in destroy().
        node.widgets.mount({
            name: 'fast_preview_batch',
            render(container) { container.appendChild(root); },
            destroy() {
        ...
        views.set(node.id, { tryLoad, setStatus });
    });

    b.onExecuted((node, result) => {
        const view = views.get(node.id);
        if (!view || !result.raw.kj_batch_preview) return;
        const info = result.raw.kj_batch_preview[0];
        if (!info) return;
        node.setProperty('kj_batch_preview', info);
        view.tryLoad(info);
    });

    b.onConfigured((node) => {
        const view = views.get(node.id);
        const info = node.getProperty('kj_batch_preview');
        if (!view || !info) return;
        view.tryLoad(info, () => {
            view.setStatus('Preview unavailable (re-run to regenerate)');
            node.setProperty('kj_batch_preview', undefined);
        });
    });

    // The DOM teardown moved into the mount's destroy(); only the per-node
    // entry is left to release here.
    b.onRemoved((node) => {
        views.delete(node.id);
    });
});
