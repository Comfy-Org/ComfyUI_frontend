...
const GENERATED_AGENT_NAMES = new Set(
  [
    ...readFileSync(
      new URL('../../packages/ingest-types/src/index.ts', import.meta.url),
      'utf8'
    ).matchAll(/^\s+(Agent\w+),$/gm)
  ].map((match) => match[1])
)
...
function missingAgentImportMessage(name: string): string {
  return `'${name}' is generated by ${GENERATED_PACKAGE}. Import and use the generated type instead of declaring a frontend copy — see docs/guidance/typescript.md.`
}
...
export const noDuplicateIngestType = {
  create(context: RuleContext) {
    ...
    const checkInterface = (node: InterfaceDeclaration) => {
      ...
      if (bindings.size === 0) {
        if (GENERATED_AGENT_NAMES.has(node.id.name)) {
          context.report({
            node: node.id,
            message: missingAgentImportMessage(node.id.name)
          })
        }
        return
      }
