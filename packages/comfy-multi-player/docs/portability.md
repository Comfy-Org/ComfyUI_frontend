...
**Not vectored, but normative:** the schema §10 read gate (`migrate()`, refined by schema Amendment A3) is a conformance obligation the manifest cannot express. The vectors are all `applyOps` cases, so a runner in another language can pass 100% of them while accepting a document with no readable `meta.schema_version` as current — precisely the fail-open KA-11 forbids and A3 closed. Until a schema-version rejection vector exists, a conforming reader must implement A3 from the prose: an unreadable or disagreeing `meta.schema_version` is rejected, and both the rejection and the current-version no-op leave the encoded document unchanged.
