...
/** @param routerBaseUrl The Router origin whose storage grants the upload. */
export function createWorkshopUrlUploader(routerBaseUrl: string) {
  ...
  return async function upload(
    ...
    const { urls, expiresAt } = await uploadStep(
      ...
      callerSignal,
      async () => {
        const response = await fetch(`${routerBaseUrl}/customers/storage`, {
          method: 'POST',
          credentials: 'omit',
          redirect: 'error',
          headers: {
