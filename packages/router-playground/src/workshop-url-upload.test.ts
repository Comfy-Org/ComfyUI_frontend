...
const WORKSHOP_ROUTER_BASE_URL = 'https://router.example.com'
...
describe('URL upload transport', () => {
  it('uploads a video larger than 25 MiB without putting its bytes in JSON', async () => {
    ...
    expect(
      await createWorkshopUrlUploader(WORKSHOP_ROUTER_BASE_URL)(
        file,
        'token',
        'scope',
  ...
  it('identifies a browser upload transport failure without retaining the signed URL', async () => {
    ...
    vi.stubGlobal('fetch', requests)
    const error = await createWorkshopUrlUploader(WORKSHOP_ROUTER_BASE_URL)(
  ...
  it('rejects an invalid explicit expiry before uploading', async () => {
    ...
    vi.stubGlobal('fetch', requests)
    await expect(
      createWorkshopUrlUploader(WORKSHOP_ROUTER_BASE_URL)(
