...
// PM-1157 / PM-1158: dragging a non-first output of a multi-output/batch job
// (the "layers" stack expansion in the asset sidebar) into the agent chat
// composer shows the wrong image once the message is sent. The asset-drag
// payload for a non-primary output has no content hash — only a bare output
// filename in `ref` (see outputAssetUtil.ts's deliberate omission) plus a
// correct `previewUrl`. UserMessage.vue's splitAttachments discards
// `previewUrl` whenever `ref` is set and reconstructs
// `/view?filename=<ref>&type=input`, which does not resolve to the dragged
// output. Canvas drop and the primary/first output are unaffected (see the
// RCA), so this reproduces the composer-only, non-first-output-only bug.
test(
  ...
  async ({ page, workflowSelection, promptHistory }) => {
    ...
    await bubble
      .locator('..')
      .screenshot({ path: 'test-results/pm-1158-batch-output-attachment.png' })
