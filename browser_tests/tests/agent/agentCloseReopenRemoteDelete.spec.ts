...
// The tab is closed, the host deletes a node while nothing is subscribed, and
// the tab is reopened from persistence — so the saved workflow on disk still
// holds the node and only the catch-up answering the new subscribe says it is
// gone. A deletion that does not stick here is the resurrection a user reads as
// "the agent undid my delete".
//
// Extracted from FE #18277, whose code half stays parked for the remote-apply
// pivot (Linear FE-2504, landing on FE #18700). This case passes on `main`
// without that code, so it lands here as a plain regression pin: it is one of
// the behaviors the pivot must not lose.
test.describe(
