...
test.describe(
  'A human canvas rename mints a set_title op',
  { tag: ['@cloud', '@agent'] },
  () => {
    test.use({ conversationCase: CASE, humanOpsHost: 'apply' })

    test('renaming a node from the canvas sends set_title, not just a local title change', async ({
      agentConversation
    }) => {
      ...
      await expect
        ...
        .toEqual([`set_title:${RENAMED_NODE_ID}`])
    })
  }
)
