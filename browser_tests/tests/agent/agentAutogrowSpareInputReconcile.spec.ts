...
test.describe(
  ...
  () => {
    ...
    // Known remaining gap, measured at `35524d57b8` with #18127 in place:
    // the reconcile restores the spare on the litegraph node (the case
    // above passes) but the rendered node keeps drawing one slot row, so
    // the user still has nowhere to drop a second image. Dumped from the
    // DOM either side of the switch: `[2-in-0 "image_1", 2-in-1
    // "image_2"]` before, `[2-in-0 "image_1"]` after, with the spare
    // present in `node.inputs` throughout. `test.fail()` keeps this red-on-
    // fix rather than silently green: whoever closes the gap deletes the
    // annotation.
