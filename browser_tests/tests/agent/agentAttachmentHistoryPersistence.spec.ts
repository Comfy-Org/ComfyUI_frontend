...
test(
  ...
  async ({ page, promptHistory, workflowSelection }, testInfo) => {
    ...
    const reopenedImage = reopenedPanel.getByRole('img', {
      name: droppedFilename
    })
    await expect(reopenedImage).toBeVisible({ timeout: 10_000 })
    await expect(reopenedImage).toHaveJSProperty('naturalWidth', 64)
