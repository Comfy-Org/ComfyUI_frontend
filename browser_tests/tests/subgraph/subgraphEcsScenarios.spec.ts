    ...
    for (const mode of modes) {
      test.describe(
        ...
        () => {
          ...
          test('blank promoted text saves and reloads with its host intact', async ({
            comfyPage
          }) => {
            ...
            const host = await clipNode.convertToSubgraph()
            const hostId = host.id
            const hostIdentity = await comfyPage.page.evaluate((hostId) => {
              const node = window.app!.rootGraph.getNodeById(hostId)
              if (!node?.isSubgraphNode())
                throw new Error('Subgraph host not found')
              return { type: node.type, subgraphId: node.subgraph.id }
            }, hostId)

            await expect
              .poll(() =>
                comfyPage.subgraph.getPromotedWidgetOrder(String(hostId))
              )
              .toContain('text')
            const promotedName = await host.getPromotedTextWidget('text')
            await expect(promotedName).toHaveCount(1)

            await test.step('user clears the promoted text value', async () => {
              await promotedName.clear()
            })

            await expect(promotedName).toHaveValue('')
            const workflowName = `${mode.label.toLowerCase()}-blank-promoted-name`
            ...
            await test.step('user saves without an error', async () => {
            ...
            const reloadedHost = await comfyPage.nodeOps.getNodeRefById(hostId)
            await expect.poll(() => reloadedHost.exists()).toBe(true)
            await expect
              .poll(() =>
            ...
            const reloadedName =
              await reloadedHost.getPromotedTextWidget('text')
            await expect(reloadedName).toHaveValue('')
            await expect
              .poll(() =>
                reloadedHost
                  .getWidgetByName('text')
                  .then((widget) => widget.getValue())
              )
              .toBe('')
          })
