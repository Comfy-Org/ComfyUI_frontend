...
test.describe(
  ...
  () => {
    ...
    test('reports rejected asynchronous example actions', async ({
      ...
      const invalidEventResponse = await comfyPage.page.request.post(
        `${comfyPage.url}/how-to-api/event`,
        { data: [] }
      )
      ...
      const previousPingError = await backendPing
        .getWidget(1)
