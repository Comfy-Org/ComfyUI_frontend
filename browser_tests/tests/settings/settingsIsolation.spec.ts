...
test('replaces previous settings before boot and preserves runtime changes on reload', async ({
  comfyPage
}) => {
  ...
  expect(await comfyPage.settings.getSetting('Comfy.Graph.ZoomSpeed')).toBe(1.5)
  expect(
    await comfyPage.settings.getPersistedSetting('Comfy.Graph.ZoomSpeed')
  ).toBe(1.5)
})

test('manual boot settings use the same baseline and cannot redirect the target user', async ({
  comfyPage
}) => {
