...
// https://linear.app/comfy-org/issue/PM-1342
//
...
test.describe(
  'Agent-generated video asset (PM-1342)',
  { tag: '@cloud' },
  () => {
    ...
    test('renders a real thumbnail and a playable video, not a black tile', async ({
      ...
      // `thumbnail_url || preview_url || ''` fallback currently produces for
    ...
    test('dragging the video asset onto the canvas carries its workflow metadata', async ({
      ...
      // workflow metadata (see PM-1342 background - this may be a backend
      // pipeline gap rather than a frontend one; this test only pins today's
      // observable behavior). `plain_video.mp4` is the same "no embedded
      // workflow" fixture the agent chat panel mocks already use.
