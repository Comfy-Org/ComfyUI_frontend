...
test.describe('first-run tour role pins', { tag: '@workflow' }, () => {
  test('every pinned node still exists with its pinned type', async ({
    ...
    expect(
      unserved,
      'every pinned template must be served, or its card vanishes from the Getting Started grid and its pins go unchecked — the drift this guard exists to catch'
    ).toEqual([])
  })
})
