...
| 12  | Frontend extension load (story S11; s1)          | FE   | A pack's frontend JS silently fails to load - a wrong web dir, a `loadExtensions` regression - while its backend nodes stay in `/object_info`, so every JS-driven behavior vanishes and the suite would quietly test vanilla nodes | `src/services/extensionService.ts` `loadExtensions`: drop `ComfyUI-KJNodes` from the import list so its JS never loads (captured in isolation on branch `nathaniel/cap-s11` off the shipped state, so the red is attributable to this one break; CI run 30292373406) | Tests Custom Nodes / mount tier (T0 load)  | `ComfyUI-KJNodes: frontend extension "KJNodes.appearance" not registered - pack JS did not load` (`customNode.regression.spec.ts:118`)                                |
| 13  | Dynamic inputs / autogrow (story S12; s1)        | FE   | A change that stops pack JS's `onConnectionsChange` autogrow override from firing on a live connect - a class no def-driven tier can see, because the behavior is invisible to `/object_info`                                      | `src/lib/litegraph/src/LGraphNode.ts` connect: suppress the live-connect INPUT `onConnectionsChange` call the pack overrides (captured in isolation on branch `nathaniel/cap-s12` off the shipped state; CI run 30292391836)                                         | Tests Custom Nodes / dynamic-inputs tier   | `ImpactMakeImageList via drag with VueNodes=false: input count grows by one on connect` (expected 2, got 1; `dynamicInputs.spec.ts:229`)                              |
