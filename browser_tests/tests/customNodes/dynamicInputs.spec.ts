...
for (const { autogrowCase } of applicableAutogrowCases) {
  test.describe(`dynamic inputs: ${autogrowCase.pack} @custom-nodes`, () => {
    for (const vueNodesEnabled of [false, true]) {
      test(
        `${autogrowCase.consumerType} grows on connect and shrinks on disconnect (drag + programmatic, VueNodes=${vueNodesEnabled})`,
          ...
          for (const connectPath of ['drag', 'programmatic'] as const) {
            ...
            if (vueNodesEnabled)
              await expect
                .poll(
                  async () =>
                    (await consumerShape(comfyPage.page, consumerId))
                      .domSlotDots,
