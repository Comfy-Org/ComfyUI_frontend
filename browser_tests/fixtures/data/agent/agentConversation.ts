...
// `mintedIds`, `OP_ENVELOPE_KEYS`, `zAgentConversationWorkflow` and
// `zAgentConversationRequest` are module-local on this branch. `main` exports
// them for `scripts/agentConversationAssemble.ts`, the recording-authoring
// tool, which is not backported here; re-export them if it ever is.
//
...
export const zRecordedWsEvent = z.discriminatedUnion('type', [
  ...
  messageDraft.extend({ data: messageDraft.shape.data.omit(mintedIds) }),
