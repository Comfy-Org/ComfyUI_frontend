...
// Two of these join the whole published corpus rather than a fixture, which
// takes seconds rather than milliseconds, so they say so instead of sitting
// just under the default budget and crossing it on a loaded runner.
...
describe('buildTemplateModelJoin', () => {
  ...
  it(
    'keeps HappyHorse operations and versions distinct and never links chat to an image model',
    () => {
      ...
      expect(useCaseForTemplate(reference, workshopModels)).toBe(
        'animate-images'
      )
    },
    CORPUS_TIMEOUT
  ...
  it(
    'reproduces canonical targets that all exist in the published catalogue',
    () => {
