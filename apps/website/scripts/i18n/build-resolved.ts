...
import {
  buildResolvedDictionary,
  buildPluralFallbackKeys
} from '../../src/i18n/pipeline/resolved'
...
function main(): void {
  ...
  const pluralFallbacks: Partial<Record<Locale, string[]>> = {}

  for (const locale of LOCALE_CODES) {
    const machine = readMachineLayer(locale)
    pluralFallbacks[locale] = buildPluralFallbackKeys(entries, locale, machine)
    const dictionary = buildResolvedDictionary(entries, locale, machine)
    fs.writeFileSync(
  ...
  fs.writeFileSync(
    path.join(I18N_DIR, 'plural-fallbacks.json'),
    `${JSON.stringify(pluralFallbacks, null, 2)}\n`,
    'utf8'
  )
}
