#!/usr/bin/env bash
# Drives every fix-it eval case's fixture through the sequence the case
...
read_rest() { ./bin/gh pr checks 4242 >/dev/null; ./bin/gh api graphql -f query=x >/dev/null; ./bin/gh api repos/o/r/issues/4242/comments >/dev/null; }
...
./bin/gh pr checks 4242 >/dev/null; ./bin/gh api graphql -f query=x >/dev/null
