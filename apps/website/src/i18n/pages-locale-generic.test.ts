...
// These translated pages landed on main after the shared-page migration began.
const dedicatedEnterprisePages = new Set([
  join('zh-CN', 'enterprise', 'index.astro'),
  join('zh-CN', 'enterprise', 'managed-builds.astro')
])
...
describe('page files never name a locale', () => {
  const pages = astroPages(pagesDir)
    .map((file) => ({
      name: relative(pagesDir, file),
      body: withoutStaticPaths(readFileSync(file, 'utf8'))
    }))
    .filter(({ name }) => !dedicatedEnterprisePages.has(name))
