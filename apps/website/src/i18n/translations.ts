...
Enterprise`
  ...
  'models.faq.isFree.cloudAnswer': {
    en: "This model runs exclusively on Comfy Cloud and is billed in credits consumed per use. Credit cost varies by model and isn't broken out per-model on this page yet - see comfy.org/cloud/pricing for current plans and how credits work.",
    'zh-CN':
      '此模型仅在 Comfy Cloud 上运行，按使用量以积分计费，不同模型的积分消耗有所不同，目前本页面尚未列出各模型的具体积分消耗。当前套餐及积分使用方式请参阅 comfy.org/cloud/pricing。'
  },
