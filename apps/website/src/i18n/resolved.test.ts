...
import pluralFallbacks from './plural-fallbacks.json'
...
describe('resolved dictionaries match the source they came from', () => {
  ...
  it.for(LOCALE_CODES)(
    '%s preserves the source language for plural fallback',
    (locale) => {
      const expected = translationKeys.filter((key) => {
        const resolved = resolveTranslation(key, locale)
        return (
          locale !== 'en' &&
          resolved.provenance === 'english' &&
          resolved.value.includes('|')
        )
      })
      expect(pluralFallbacks[locale]).toEqual(expected)
    }
  )
