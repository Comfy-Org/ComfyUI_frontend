...
import { buildResolvedDictionary, buildPluralFallbackKeys } from './resolved'
...
describe('plural fallback language', () => {
  const plurals: SourceEntry[] = [
    { key: 'fallback', english: '{count} node | {count} nodes', approved: {} },
    {
      key: 'approved',
      english: '{count} node | {count} nodes',
      approved: { ja: '' }
    },
    { key: 'machine', english: '{count} node | {count} nodes', approved: {} },
    { key: 'plain', english: 'Nodes', approved: {} }
  ]

  it('uses English rules only for untranslated plural messages', () => {
    expect(
      buildPluralFallbackKeys(plurals, 'ja', { machine: '{count}個' })
    ).toEqual(['fallback'])
  })

  it('needs no fallback override for the English dictionary', () => {
    expect(buildPluralFallbackKeys(plurals, 'en', {})).toEqual([])
  })
})
