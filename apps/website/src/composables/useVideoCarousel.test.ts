...
describe('useVideoCarousel', () => {
  ...
  it('skips over an already-failed slide and still reports forward travel', async () => {
    const { carousel } = await mountCarousel(3)
  ...
  it('retries from the start once every slide has failed', async () => {
    ...
    expect(
      ...
      'the third failure exhausts the set, which clears it and lands back on slide 0'
    ).toBe(0)
...
describe('wrapIndex', () => {
  it.for([
    ...
    [-1, 3, 2],
    [4, 3, 1]
  ])('wraps %i within %i to %i', ([value, length, expected]) => {
