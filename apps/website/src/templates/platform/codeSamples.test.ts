...
describe('routerCodeTabs', () => {
  it('cycles every supported provider in each language sample', () => {
    const providers = ['fal', 'runware', 'wavespeed', 'higgsfield']
  ...
  it('copies as a working Router request for the chosen provider', () => {
    expect(resolve(routerCodeTabs.curl.segments, 0)).toBe(
    ...
    expect(resolve(routerCodeTabs.python.segments, 3)).toContain(
      'provider="higgsfield"'
    )
  })
})
