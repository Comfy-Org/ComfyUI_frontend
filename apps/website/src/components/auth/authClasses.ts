/**
 * The cloud onboarding pages' brand Button variants and the PrimeVue Aura
 * dark rules their components inherit there and have to be spelled out here;
 * the shared treatments come from @comfyorg/account/authClasses.
 */
import * as shared from '@comfyorg/account/authClasses'
...
/**
 * The shared field class on top of PrimeVue's InputText base and Aura's dark
 * form-field tokens (zinc borders, blue-400 focus, red-300 invalid).
 */
export const AUTH_FIELD_CLASS = cn(
  shared.AUTH_FIELD_CLASS,
  'w-full appearance-none border border-[#52525b] text-base shadow-[0_0_#0000,0_0_#0000,0_1px_2px_0_rgba(18,18,23,0.05)] transition-[background,color,border-color,outline-color,box-shadow] duration-200 outline-none hover:border-[#71717a] focus:border-[#60a5fa] aria-invalid:border-[#fca5a5] aria-invalid:placeholder:text-[#f87171]'
)
