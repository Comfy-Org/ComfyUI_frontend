...
export function createWorkflowMotion(
  ...
  const plans = workflows.map((workflow) => {
    ...
    for (const step of workflow.steps) {
      ...
      if (step.type === 'pause') {
        ...
        if (step.from) {
          state.frames[0].value =
            'opacity:0;transform:translate(0,0) scale(.96);'
          const from = {
            x: step.from.x + point.x - node.x,
            y: step.from.y + point.y - node.y
          }
          const translate = `translate(${((step.from.x - node.x) / width) * 100}cqw,${((step.from.y - node.y) / width) * 100}cqw) scale(1)`
          state.frames.push(
          ...
          time += duration
          reveal(state, time + 0.1)
        }
        event.dropAt = state.shownAt
        click(cursor, time)
        time = Math.max(time + 0.2, state.shownAt ?? time)
        if (step.actor === 'user') {
      ...
      } else if (step.type === 'connect' || step.type === 'connect-group') {
        ...
        if (
          ...
          step.scale <= 0 ||
          !Number.isFinite(step.offset.x) ||
          !Number.isFinite(step.offset.y)
        )
          throw new Error(
  ...
  const scenes = plans.map((plan) => {
    ...
    const nodes = plan.nodes.map((state) => {
      ...
      const imageName =
        state.imageAt === undefined ? undefined : `${name}-image`
      if (imageName && state.imageAt !== undefined)
        rules.push(keyframes(imageName, opacityFrames(state.imageAt, 0.2)))
      const selectionName =
        state.selectionAt === undefined ? undefined : `${name}-selection`
      if (selectionName && state.selectionAt !== undefined)
        rules.push(
          keyframes(selectionName, opacityFrames(state.selectionAt, 0.05))
        )
      const promptName =
        state.promptAt === undefined ? undefined : `${name}-prompt`
      if (promptName && state.promptAt !== undefined)
        rules.push(keyframes(promptName, opacityFrames(state.promptAt, 0.01)))
