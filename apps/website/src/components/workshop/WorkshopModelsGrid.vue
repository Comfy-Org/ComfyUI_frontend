...
const capabilityOptions = computed<FacetMenuOption[]>(() => {
  ...
  for (const model of withinSection.value)
    for (const value of categoriesFor(model))
...
<template>
  <section class="gap-10">
    ...
    <div class="min-w-0">
      ...
      <div
        ref="toolbar"
        class="bg-page sticky top-20 z-30 -mx-1 mb-8 flex scroll-mt-20 flex-wrap items-center justify-end gap-3 px-1 py-4 max-sm:mb-4 max-sm:py-2 sm:flex-nowrap lg:top-26 lg:scroll-mt-26"
      >
        <h1
          v-if="inSection"
