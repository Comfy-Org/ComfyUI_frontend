...
>(config: BuildDataSourceConfig<TOptions, TSnapshot, TFreshData>) {
  ...
  function fetchForBuild(
    ...
    const resolvedOptions = options ?? config.getDefaultOptions()
    const cacheKey = config.getCacheKey?.(resolvedOptions) ?? 'default'
    if (inflight) {
    ...
    inflight = fetchBuildData(config, resolvedOptions)
    return inflight
  }
