...
function buildAnnotations(outcome: FetchOutcome): string[] {
  ...
  if (outcome.status === 'stale') {
    return [
      `::warning title=GitHub stars unavailable::${escapeAnnotation(outcome.reason)}. Using last-known-good snapshot.%0A%0AAction items:%0A  1. Check GitHub API availability and rate limits.%0A  2. Re-run the build once GitHub is healthy.%0A  3. To refresh the fallback value, run \`pnpm --filter @comfyorg/website github-stars:refresh-snapshot\` and commit apps/website/src/data/github-stars.snapshot.json.`
    ]
  }

  return [
    `::error title=GitHub stars fetch failed and no snapshot is available::Cannot render website navigation stars.%0A%0AReason: ${escapeAnnotation(outcome.reason)}%0A%0AAction items:%0A  1. Run \`pnpm --filter @comfyorg/website github-stars:refresh-snapshot\`.%0A  2. Commit apps/website/src/data/github-stars.snapshot.json.%0A  3. Push and re-run CI.`
  ]
}
