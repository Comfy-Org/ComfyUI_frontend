...
- **Two in a row.** Buy credits twice in quick succession. Each purchase is charged once, and two distinct rows appear in `Activity` with the right amounts.
- **Immediately check the log.** Buy credits, then open `Activity` straight away. The top-up appears with the correct amount.
...
- **The usage log will not show a team top-up.** `GET /api/billing/events` with a team-scoped token returns `total: 0` twelve minutes after two settled top-ups, while the personal scope lists `credit_added` events. This is a backend gap. Do not file it as a rail defect.
