...
| `getCustomWidgets` POJO                  | `defs.defineWidgetType(type, { render })` | preserves the backend-declared widget's wire cell                         |
...
Prefer `comfy.supports('...')` over version comparisons — see
`docs/node-api/registration.md`.

### 4. Watch for the frame-to-event shift

A draw callback recomputed from current state every repaint, so nothing ever
needed to announce a change. Canvas surfaces must be **redrawn when the value
changes**. A port that calls `redraw()` from inside the old callback body will
appear to work and quietly run on every repaint forever.
...
- The published API has no destination. A missing API is a **core gap to file**,
  not something to work around in a pack. `docs/node-api/README.md` defines the
  contract boundary.
...
no arbitrary properties. **Keep a `Map` keyed by
`${node.graphId}:${node.id}`** and clear the entry in `b.onRemoved`. This is
supported, not a workaround; the old property was collected with the node and
a Map is not.
...
is a design decision, not a detail. Record the tradeoff and rejected alternatives
in an ADR following `docs/adr/README.md`, then update `docs/node-api/`. An
addition made under conversion pressure with its rationale only in a commit
message is one nobody can argue with later.
...
1. **How common it is**, measured — not estimated. Counts come from the current
   registry corpus, should record the dataset pin, and should be labelled
   grep-derived where they are.
...
If a pattern turns out to be mechanically safe, add it to Magic Patch's rule
catalog instead: a rule with golden cases beats prose, because CI runs it.
Prose is for the judgement calls.
...
When Magic Patch tooling is present, keep mechanically safe rewrites and their
golden cases in its rule catalog. This skill is for the intent and judgement
that cannot be encoded as a mechanical rule.
