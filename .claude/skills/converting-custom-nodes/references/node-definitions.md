...
Custom output keys survive in `raw` — ADR-NODE-OUTPUTS-0007's passthrough
schema guarantees it — so a pack reading a bespoke key keeps working.
...
Every read and write on a `NodeHandle` is a method — the contract generated by
`scripts/node-api/gen_api_dts.mjs`, so a read can be a store query and a write
can become a command. Property syntax silently does nothing or throws.
...
the state yourself, keyed by graph scope and node id:
...
const stateFor = (node) => {
  ...
  let s = stateByNode.get(key)
  if (!s) stateByNode.set(key, (s = {}))
  return s
}
...
comfy.defs.extend('MyNode', (b) => {
  ...
  b.onRemoved((node) => stateByNode.delete(stateKey(node)))
})
```
...
Prototype-assignment counts derive from the 4,969-pack registry corpus used
when this reference was authored (2,562 files scanned). Grep-derived;
sample-verify before citing an individual number.
