...
└─ yes
   ├─ small status label or badge                           → node.addBadge()
   ├─ custom text, bar, border, tint, image, or icon        → widgets.canvas()
...
For a small status label that belongs in node chrome, use `node.addBadge()`:
...
Keep the returned removal function when the badge has a shorter lifetime than
the node. The callback runs during drawing, so it must remain fast.
...
For custom drawing, the shipped destination is **`node.widgets.canvas()`** — a
per-node drawing surface that works under both renderers, because the canvas is
a DOM element the legacy renderer positions over the graph and Nodes 2.0
renders natively:
...
b.onCreated((node) => {
  const surface = node.widgets.canvas({
    ...
    draw(ctx) {
      ctx.font = 'bold 12px sans-serif'
      ctx.fillText(stateFor(node).value ?? '', 4, 15)
    }
  })
  stateFor(node).surface = surface
})
b.onExecuted((node, result) => {
  stateFor(node).value = String(result.text[0] ?? '')
...
`node.addBadge()` is the declarative destination for simple labels;
`widgets.canvas()` remains the destination for arbitrary pack drawing.
...
Measured over the 4,969-pack registry corpus used when this reference was
authored. Counts are grep-derived and have a known false-positive rate —
sample-verify before citing any individual number.
