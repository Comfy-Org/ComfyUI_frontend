...
> **Accessor style.** The published handles follow the generated declaration
> from `scripts/node-api/gen_api_dts.mjs`: reads and writes are methods, not properties —
...
- Hiding purely for presentation → `setHidden(true)` is complete.
- Hiding _and_ suppressing serialization → call `setHidden(true)`, and handle
...
pair) becomes an explicit relationship. `widget.setLinked(names)` records it,
`widget.linked()` reads it, and `setHidden()` cascades to linked widgets. The
recursive hide bookkeeping goes away.
...
hand-roll `{ ...widget.getOptions(), ...patch }` you will invoke the getter and
freeze its result — pinning a dynamic combo to a one-time snapshot, silently.

`getOptions()` returns a frozen _snapshot_ by design; use `setOption` to write.

## `getCustomWidgets` — 91 packs

Register one renderer for the backend-declared input type:

```js
comfy.defs.defineWidgetType('MY_PACK_SLIDER', {
  defaultValue: 0,
  height: 40,
...
`render` receives a host-owned container and a value handle. Packs may use
their own framework or plain DOM, and must return teardown for listeners,
timers, and observers. Defining the type preserves the backend input's original
positional `widgets_values` cell; adding a second mounted widget does not.
...
| `setHidden(true)` alone, where the old code also suppressed serialization | Wire-format change                               |
...
Counts from the 4,969-pack registry census used when this reference was
authored. Grep-derived with a known false-positive rate — sample-verify before
citing an individual number.
