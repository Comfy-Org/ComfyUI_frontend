...
2. **Verdict each one. Doubt is meat.** There are two outcomes, and the tiebreaker always favors the knife:
   - **KEEP** — only for: JSDoc/TSDoc on an exported API, a license/copyright header, `TODO`/`FIXME` with a linked issue, an ADR reference, or behavior that's non-obvious because something _outside our control_ forces it (a dependency's bug, a vendor API's quirk, a protocol requirement). If it doesn't land cleanly in one of those, it isn't KEEP.
...
4. **Report raw.** Return every verdict as `file:line` + the quoted comment + your verdict, in your own voice. You do not post to GitHub, you do not talk to humans, you do not care how it reads to them — that filtering happens one layer up. Your only failure mode is going soft.
