...
**The axis is the key kind, not the store.** Identity-keyed stores hold
app-minted counters (`NodeId`, `LinkId`, `RerouteId`); a collision there is two
distinct entities claiming one name, and merging them would be data
corruption, so those stores reject. The structurally keyed store derives its
key from position (`graphId:nodeId:name`); same position plus same type _is_
the same logical slot, so that store resolves. Neither rule can be applied
uniformly: resolve-everywhere lets a colliding node silently steal an
incumbent's links and widget state, while reject-everywhere makes ordinary
node replacement (reload, paste, kind change) strand or lose widget values
that position keys legitimately recycle.
...
_same_ object under the same owner is idempotent and returns the incumbent.
The loser stays detached, so its later writes and its removal cannot corrupt
the winner's registration. Diagnostics differ by recovery ownership:
`linkStore` and `rerouteStore` log a `console.error` at the store because no
caller resolves the collision, while `nodeDataStore` is deliberately silent —
its rejection is control flow for the remint loop (below), which owns the
diagnostic. Store-level rejection is not an invariant violation: serialized,
pasted, and imported ids are observed input and may legitimately collide.
Lifecycle corruption is different and asserts: re-registering an
already-registered node under a different root graph, or unregistering a
state the store does not hold, throws in DEV (`registerNodeState` /
`unregisterNodeState` in `src/core/graph/nodeShell/nodeShellState.ts`).

**The derived-key store — `widgetValueStore` — resolves.** `WidgetId` is
`graphId:nodeId:name`, computed rather than minted, so "same id" means "same
widget" by construction. Re-registering with the same `type` returns the
incumbent state, so widget values survive node replacement. Re-registering
with a different `type` is a legitimate re-mint (the node definition changed),
not two entities colliding, and overwriting with the newcomer is the correct
resolution. Converging this store on rejection would break widget re-mint;
converging the other three on overwrite would violate one-state-per-entity.
**Neither direction is a safe unification.** (If widget identity is later
split into an immutable `kind` and a mutable presentation type — proposed
ADR-0016 — the discriminator in this contract is the immutable kind; a
presentation change must not trigger the overwrite path.)
...
**Collision recovery lives at the remint site, not in the store.** The remint
loop lives in `attachNodeToStores`
(`src/core/graph/nodeShell/nodeShellLifecycle.ts`, extracted from `LGraph.add`
by #14257), which retries with a freshly minted id until registration
succeeds; `LGraph.add` is its only production caller. The loop is the one
place where both the colliding and the replacement ids are in scope, so the
diagnostic belongs there: a `console.warn` naming both ids and the root graph
(landed with #15720). A warning, not an error, because the path is reachable
on normal loads (subgraphs share the root id counter; configure and paste ids
...
third verb that only helps callers that already know they are displacing, and
every collision-prone caller today (configure, paste) does not know: they
observe ids from files. The uniform contract lives one level up: every store
has an explicit, documented, tested collision contract derived from its key
kind.
...
(`nodeShellState.ts`, used by `attachNodeToStores`), `registerLinkTopology` /
`replaceLinkTopology` (`LLink.ts`), and `registerRerouteChain` (`Reroute.ts`).
Every production call site goes through one of these, and the remint loop in
`attachNodeToStores` is the only caller that acts on rejection. A richer
`registered | alreadyRegistered | conflict` union would add API surface
without changing any decision a current caller can make; revisit it when a
second caller needs the distinction.
...
`SubgraphNode` call it directly — and its `undefined` means _un-keyable id_,
not _collision_. `setNodeId` treats it as "keep my local state", leaving the
widget silently detached from the store. Do not read `undefined` from one of
these five functions as though it carried the meaning it has in the others.

**Boundaries.** Remint is a local resolution (file load, paste) and never a
multiplayer reconciliation mechanism; cross-client duplicate-id
reconciliation belongs at the CRDT merge boundary (ADR-0003, "Amendment
(2026-08-23): merge-boundary reconciliation constraint", which also carries
the corrected per-store table). The per-store contracts are pinned by
