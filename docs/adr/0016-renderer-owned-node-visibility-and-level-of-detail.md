# 16. Nodes 2.0 Viewport Retention and Level of Detail
...
The implementation has three renderer concerns:

- viewport membership derives attached node IDs from camera and layout data;
- pin collection derives lifecycle-sensitive node IDs from explicit events;
- slot readiness waits only for nodes currently attached to the DOM.
...
current activation threshold and avoids a second geometry cache, invalidation
contract, admission queue, and timing state machine. The low-quality box layer
may query the canonical `layoutStore` index because it already owns that
geometry.
...
- Visibility policy, safety pins, and slot readiness are independently
  testable.
