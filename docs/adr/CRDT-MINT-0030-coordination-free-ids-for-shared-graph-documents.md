# ADR-CRDT-MINT-0030: Coordination-Free Ids for Shared Graph Documents
...
[ADR-CRDT-MINT-0018](CRDT-MINT-0018-merge-identity-for-node-transfers.md) chose merge-boundary
...
([ADR-CRDT-FOLLOWER-0025](CRDT-FOLLOWER-0025-in-app-agent-crdt-follower-and-distribution-resolved-boundaries.md)) keys the shared
...
ADR-CRDT-MINT-0018 named actor-scoped ids (`actor:counter`) as the durable fix. That remains the
...
3. **Random, not actor-scoped.** Ids stay `number`, which is what makes this shippable
   ...
   ADR-CRDT-MINT-0018's own remint-on-reject path in `LGraph.add`).
...
- This does not make ids globally unique at creation in the sense ADR-CRDT-MINT-0018 meant.
...
ADR-CRDT-MINT-0018's recorded plan: `actor:counter` string ids, collision-free by construction.
...
- [ADR-CRDT-MINT-0018](CRDT-MINT-0018-merge-identity-for-node-transfers.md) — merge-boundary reminting
  and the revisit trigger this ADR answers
- [ADR-GRAPH-DOCUMENT-0024](GRAPH-DOCUMENT-0024-graph-activation-and-document-objects-for-in-app-agent-targets.md) — graph activation and document
  objects for agent targets
- [ADR-CRDT-FOLLOWER-0025](CRDT-FOLLOWER-0025-in-app-agent-crdt-follower-and-distribution-resolved-boundaries.md) — the follower that
