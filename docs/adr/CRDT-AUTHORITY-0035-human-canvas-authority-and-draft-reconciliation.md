...
(`agentWorkflowTabBindingStore.ts`, `useWorkflowPersistenceV2.ts`). Every
scratch tab shares one default path, so a fresh blank scratch tab matched a
persisted binding exactly like a restored draft would, inherited that
binding's `workflow_id` before the user typed anything, and painted the old
document onto the blank canvas. The posted draft then agreed with the stale
document, so no server-side reconciliation could help: the frontend had
resolved the wrong workflow.
...
4. **A lineage break settles pending human operations.** When `doc_reset`
   arrives for the bound document, the sender settles its in-flight and queued
   batches `undeliverable` at once, symmetric to the unbind case. Those
   operations were minted against the old lineage and the draft that caused
   the reset already carries their effect.
5. **A persisted scratch binding is adopted only by a restored draft.**
   Workflow persistence marks the tabs it recreates from persisted drafts
   (`useRestoredWorkflowTabStore`), and the binding store adopts a temporary
   tab only when it carries that mark. A blank, newly created scratch tab at
   the same path resolves to no workflow, so it neither subscribes to nor
   posts against the old document. Saved tabs are unaffected; explicit
   `bind()` calls are unaffected.
...
- _Do not persist temporary-tab bindings, or give scratch tabs unique paths._
  The first breaks restored drafts; the second is a workflow-store-wide
  behavior change for an agent-only concern.
- _Have the binding store infer "restored" from tab content or creation
  arguments._ Imported and job-opened tabs are also created with content, and
  a restored draft can be blank; only the persistence layer knows which tabs
  it restored.
...
- The second door is closed on the frontend alone: a blank scratch tab no
  longer inherits a stale binding, before or after the backend change.
...
- Coalescing shares one batch's prefix-abort semantics across operations that
  ...
  `unprocessed`.
- Human operations settled `undeliverable` (at unbind, at `doc_reset`, on a
  dead transport) are still reported only to the dev panel; surfacing them to
