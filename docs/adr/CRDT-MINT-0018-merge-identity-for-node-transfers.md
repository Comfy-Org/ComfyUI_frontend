# ADR-CRDT-MINT-0018: Merge Identity for Node Transfers
...
ADR-CRDT-LAYOUT-0003's merge-boundary reconciliation amendment (2026-08-23) already
...
The CRDT-based collaboration direction (ADR-CRDT-LAYOUT-0003) and the ECS store migration
(ADR-ECS-0008) break that assumption. Two replicas can each independently mint
...
ADR-ECS-0008 defines two collision contracts:
...
3. The **future CRDT boundary** is the semantic-operation applier described by
   ADR-CRDT-LAYOUT-0003. Nodes, widgets, links, and reroutes are not yet CRDT-replicated.
...
from the same merged operation set on every replica. ADR-CRDT-LAYOUT-0003 requires
...
- ADR-CRDT-LAYOUT-0003 — CRDT Layout Intent and Local Measurement (merge-boundary
  reconciliation amendment, 2026-08-23). Cite ADR-CRDT-LAYOUT-0003 externally; this ADR
  is the derivation record.
- ADR-ECS-0008 — Entity Component System (identity and structural collision
  contracts).
- ADR-ECS-SLOTS-0017 — ID-Based Slot Records Own Slot State (the same
