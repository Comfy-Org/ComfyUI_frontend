...
Resolvers are pure. They may return directly or return a promise; prompt
execution awaits async resolvers concurrently with a deadline. Synchronous
editor inspection cannot await a promise and treats it as unresolved. Resolvers
do not edit links, remove nodes, or mutate a prompt draft. See [Execution and
resolution](./execution.md) for the complete contract.
