...
`PromptInputProjection`, `PromptInputProjector`, `NodeMenuItem`,
`NodeSubMenuItem`, and `NodeColor`.
...
It may also declare `onPromptSerialize`, which returns a
`PromptInputProjection` directly or in a promise. A projection names inputs to
omit from that prompt build.
...
The generic `SlotCollection` interface backs both input and output collections.
...
`severity`, `detail`, and lifetime in milliseconds. `PlaySoundDef` contains a
browser-readable `src` and an optional volume from 0 to 1.
...
The returned handle type is `DialogHandle`.
...
The returned handle type is `MenuHandle`.
...
`SystemMonitorSnapshot` contains CPU utilization, total and available memory,
volumes, and accelerators. Unsupported utilization and temperature sensors are
`null`. Volume and accelerator IDs are opaque. Related types are
`SystemMonitorCpu`, `SystemMonitorMemory`, `SystemMonitorVolume`, and
`SystemMonitorAccelerator`.
...
| Services             | `settingsHandle.ts`, `storageHandle.ts`, `commandsHandle.ts`, `uiHandle.ts`, `backendHandle.ts`, `workflowHandle.ts`, `systemHandle.ts` |
