...
**No-flag reason:** every item is a direct correction to already-broken, already-shipped behavior (data corruption, an XSS/HTML-injection gap, a blank render, or dropped text) — the "off" state a flag would gate to _is_ the bug. There is no new user-visible behavior to ramp, no percentage rollout, and no cohort to stage. This matches the task framing for this workstream: "rollout" here is "merge each fix PR," not a ramp.
