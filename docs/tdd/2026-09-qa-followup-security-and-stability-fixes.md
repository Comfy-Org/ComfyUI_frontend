...
| Field | Value |
|---|---|
| Outcome | Six specific, QA-confirmed defects in the 1.54 release are fixed: three sanitization/XSS gaps are closed, group-node-to-subgraph conversion no longer corrupts widget values, the Assets/Models virtual grid no longer goes blank on filter or resize, and the execution-progress preview widget no longer drops bracket-delimited text such as LoRA tags. |
| Acceptance criteria | 1. A markdown image/link `title` containing `"` cannot inject additional HTML attributes or tags into rendered markdown (`markdownRendererUtil.ts`). 2. `SanitizedHtml.vue` output never contains a `<form>` element or a `style` attribute, for any input. 3. A registry-supplied `repository` URL or license `href` with a `javascript:` scheme is never rendered as a clickable `javascript:` link in `DescriptionTabPanel.vue`. 4. Converting a group node with two or more interior nodes that share a widget name (`convertToNodes()` in `groupNode.ts`) assigns every widget its own originating value — the previously-pinned `test.fail()` case in `browser_tests/tests/groupNode.spec.ts` (from PR #17464) passes as a real assertion. 5. `VirtualGrid.vue` keeps rendering items after the backing list shrinks below the previously-scrolled-to index, and after the column count grows while scrolled deep — the two `it.fails` pins in `src/components/common/VirtualGrid.test.ts` (from PR #17626) pass as real assertions. 6. `TextPreviewWidget.vue` renders literal bracket-delimited text (e.g. `<lora:name:0.8>`) as visible escaped text instead of silently dropping it. |
...
> **Provenance rule.** Every root-cause and fix-approach claim above is sourced either from reading the named file directly, from the two existing pinned-test PRs (#17464, #17626) quoted where relevant, or — for the VirtualGrid fix specifically — from the in-progress diff already on branch `fix/virtual-grid-clamp-blank` at the time this document was written. Nothing here is inferred product behavior; there is no product/billing behavior in this workstream to source.
...
|---|---|---|---|---|
| Markdown `title` breakout is closed | AC1 | Unit test asserting a `title` containing `"` cannot add an extra attribute/tag to the rendered `<img>`/`<a>` | `src/utils/markdownRendererUtil.test.ts` (fix PR adds this) | PR |
...
| `javascript:` hrefs from registry data are neutralized | AC3 | Unit test on the new scheme-check helper, plus a component test asserting `DescriptionTabPanel.vue` never renders a `javascript:` `href` | `DescriptionTabPanel.test.ts` / new util test (fix PR adds this) | PR |
| Group-node widget pairing survives conversion when names collide | AC4 | The existing `test.fail()`-pinned case in `browser_tests/tests/groupNode.spec.ts` (PR #17464) is converted to a real, passing assertion | `browser_tests/tests/groupNode.spec.ts` | PR (E2E) |
...
**No-flag reason:** every item is a direct correction to already-broken, already-shipped behavior (data corruption, an XSS/HTML-injection gap, a blank render, or dropped text) — the "off" state a flag would gate to *is* the bug. There is no new user-visible behavior to ramp, no percentage rollout, and no cohort to stage. This matches the task framing for this workstream: "rollout" here is "merge each fix PR," not a ramp.
...
rollout:
  ships_dark: false
  flags: []
  no_flag_reason: "Direct fixes to already-broken/insecure behavior (data corruption, XSS, blank render, dropped text) with no new user-visible behavior to gate; see Part 4."
