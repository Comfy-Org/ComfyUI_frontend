...
is already registered. If a different runtime link supplies an id that is
already owned, registration logs an error, leaves the incoming link detached,
and returns `undefined`. It does not silently replace the live link or assign
the incoming link another identity.
