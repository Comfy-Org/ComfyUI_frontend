...
- `browser_tests/tests/vueNodes/layout/rendererToggleGeometry.spec.ts`: slot and
  ...
  repeated toggles.
- `src/renderer/core/layout/slots/syncSlotOffsets.test.ts`: measured slot
  geometry is cached in the layout store as canvas-scale-independent offsets
  relative to the node, kept separate for the collapsed and expanded states.
