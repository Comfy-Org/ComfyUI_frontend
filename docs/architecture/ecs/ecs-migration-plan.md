...
| Lifecycle            | Graph add, configure, replace, remove, subgraph release, and clear register, transfer, or release migrated state. Clear handles reentrant and failing removal callbacks; normal node removal can retain widget entries, and `configure(keep_old)` provides weaker cleanup. |
