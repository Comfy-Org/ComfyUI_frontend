...
The audit found ADR 0008, `ecs-target-architecture.md`,
...
The audit found target-slot primary-key claims in ADR 0008,
...
The ADR 0008 Link amendment, lifecycle load note, and key tables were updated.
...
inventory tables. ADR 0003's amendment and the
...
The audit found a universal graph-key and `clearGraph(graphId)` rule in ADR 0008. Actual stores differ by concern:
...
1. LinkStore primary identity, slot-accessor status, and duplicate-link load
   behavior are current in ADR 0008 and lifecycle documentation.
2. Layout keying and command scope are current in ADR 0003/0008 and inventory
   tables.
