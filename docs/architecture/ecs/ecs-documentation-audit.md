...
This is an accuracy audit, not a proposed rewrite. It distinguishes:
...
**Role:** decision authority for canonical geometry in `layoutStore`, Yjs
...
**Role:** architectural constraint and taxonomy: dedicated stores, plain-data
state, behavior outside components, graph ownership, and no god-object growth.
...
| `ecs-migration-plan.md`          | Concise completed/remaining roadmap     | **Current + target.** Rewritten by this change to remove phased commentary and link detailed claims to focused audits.                                                                                         |
...
**Contradiction:** ADR 0008’s Slot amendment says `input.link` and
...
**Correction:** replace “mirror remains” with:
...
**Contradiction:** ADR 0008, `ecs-lifecycle-scenarios.md`, and parts of
...
**Contradiction:** `output-slot-connectivity.md` Decision 2 shows a cached
...
**Correction:** replace the computed-index pseudocode with:
...
**Contradiction:** ADR 0003 says all spatial mutations use explicit commands;
...
**Contradiction:** ADR 0008 and `proto-ecs-stores.md` still include variants of
...
**Contradiction:** `reroute-chain-store.md` Scope says `Reroute.pos` is a class
...
**Contradiction:** ADR 0008 says each dedicated store embeds graph scope in its
key and offers `clearGraph(graphId)`. Actual stores differ deliberately:
...
**Correction:** replace the universal rule with:
...
**Contradiction:** `proto-ecs-stores.md` calls NodeDataStore simply root-scoped
...
**Correction:** describe the identity/lifecycle split explicitly:
...
**Contradiction:** a file named `node-badge-store.md` with `Status: Accepted`
...
**Correction:** change the record status to **Historical — reversed** and add a
...
**Contradiction:** `ecs-lifecycle-scenarios.md` labels tables “Current vs ECS”
...
**Correction:** prefix every diagram/table heading with **Current**, **Partial
bridge**, or **Target**. Keep current flows tied to stable symbols such as
...
append to, false summaries and key tables. Pseudocode must be labeled
**Illustrative target** unless it matches a named current symbol. Historical
