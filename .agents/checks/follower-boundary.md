...
description: Checks in-app-agent CRDT follower code against the load-bearing follower/distribution invariants in ADR 0019 (and ADR 0003)
...
[ADR 0019](../../docs/adr/0019-in-app-agent-crdt-follower-and-distribution.md) and the CRDT
...
Per ADR 0019 the durable follower integrates updates into the yjs-backed FE domain stores
...
- **New durable dependence on `LitegraphMutator` / snapshot-diff / `SemanticProjector`** —
  ...
  domain store, should be questioned against ADR 0019.
...
Per ADR 0019 the follower APPLY path is distribution-agnostic; surface differences
...
the shared doc — see ADR 0019')`. Flag a follower-boundary change that adds no such guard
