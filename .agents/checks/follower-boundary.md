...
description: Checks in-app-agent CRDT follower code against the load-bearing follower/distribution invariants in ADR 0024 (and ADR 0003)
...
[ADR 0024](../../docs/adr/0024-in-app-agent-crdt-follower-and-distribution.md) and the CRDT
...
Per ADR 0024 the durable follower integrates updates into the yjs-backed FE domain stores
...
- **New durable dependence on `LitegraphMutator` / snapshot-diff / `SemanticProjector`** —
  these are the disposable POC stopgap, to be mounted with the flag-gated agent panel
  when that slice lands. New code
  that treats them as the permanent seam, or extends them instead of routing state into a
  domain store, should be questioned against ADR 0024.
...
Per ADR 0024 the follower APPLY path is distribution-agnostic; surface differences
...
the shared doc — see ADR 0024')`. Flag a follower-boundary change that adds no such guard
