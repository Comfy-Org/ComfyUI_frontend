...
description: Checks in-app-agent CRDT follower code against the load-bearing follower/distribution invariants in ADR 0020 (and ADR 0003)
...
[ADR 0020](../../docs/adr/0020-in-app-agent-crdt-follower-and-distribution.md) and the CRDT
...
Per ADR 0020 the durable follower integrates updates into the yjs-backed FE domain stores
...
- **New durable dependence on `LitegraphMutator` / snapshot-diff / `SemanticProjector`** —
  ...
  domain store, should be questioned against ADR 0020.
...
Per ADR 0020 the follower APPLY path is distribution-agnostic; surface differences
...
the shared doc — see ADR 0020')`. Flag a follower-boundary change that adds no such guard
